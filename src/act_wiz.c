...
void do_gocial(CHAR_DATA *ch, char *argument)
{
    ...
    if ( !IS_NPC(ch) && IS_SET(ch->comm, COMM_NOGSOCIAL))
    {
    ...
    if (arg[0] == '\0')
    {
        sprintf(buf, "{&[GSocial]{0: %s", social_table[cmd].char_no_arg );
        act_new(buf,ch,NULL,NULL,TO_CHAR,POS_DEAD);
        sprintf(buf, "{&[GSocial]{0: %s", social_table[cmd].others_no_arg );
        for (d = descriptor_list; d != NULL; d = d->next)
        {
            ...
            if (d->connected == CON_PLAYING &&
                d->character != ch &&
                !IS_SET(vch->comm,COMM_NOGSOCIAL) &&
    ...
    {
        sprintf(buf,"{&[GSocial]{0: %s", social_table[cmd].char_auto);
        act_new(buf,ch,NULL,NULL,TO_CHAR,POS_DEAD);
        sprintf(buf,"{&[GSocial]{0: %s", social_table[cmd].others_auto);
        for (d = descriptor_list; d != NULL; d = d->next)
        {
            CHAR_DATA *vch;
            vch = d->original ? d->original : d->character;
    ...
    {
        sprintf(buf,"{&[GSocial]{0: %s", social_table[cmd].char_found);
        act_new(buf,ch,NULL,victim,TO_CHAR,POS_DEAD);
        sprintf(buf,"{&[GSocial]{0: %s", social_table[cmd].vict_found);
        act_new(buf,ch,NULL,victim,TO_VICT,POS_DEAD);

        sprintf(buf,"{&[GSocial]{0: %s", social_table[cmd].others_found);
...

{
   ...
   if (IS_NPC(victim))
    ...
    // weakness!sprintf(buf, "{!*** {&%s has been {z{!A-BOMBED{x {&by %s{!!!{0 ", victim->name, ch->name);
sprintf(buf,"\n\r\
                              {bo====o\n\r\
                              o====o \n\r\
                               {B| /| \n\r\
                        ...
                        \\=======\\/=======/ \n\r\
                             ...
                             {r\\\\ || // \n\r\
                              \\\\||// \n\r\
                               \\\\//{x \n\r\
          {m/==================================================\\ \n\r\
          |{W          For crimes against the Keepers,         {m| \n\r\
          |                    {r%-10s{m                    | \n\r\
          |{W    Has been turned into toilet paper, use it!{m    | \n\r\
          |{W Don't forget this, you may turn into TP someday.{m | \n\r\
          \\==================================================/{x \n\r",victim->name);
    do_echo(ch, buf);
...
void do_invisin( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
	...
	if (argument[0] == '\0')
	{
	    sprintf(buf,"Your invisin is %s\n\r",ch->pcdata->invisin);
	    send_to_char(buf,ch);
	    return;
	}

	free_string( ch->pcdata->invisin );
	ch->pcdata->invisin = str_dup( argument );

        sprintf(buf,"Your invisin is now %s\n\r",ch->pcdata->invisin);
        send_to_char(buf,ch);
    }
    return;
}
void do_invisout( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    ...
    if ( !IS_NPC(ch) )
    {
        ...
        if (argument[0] == '\0')
        {
            sprintf(buf,"Your invisout is %s\n\r",ch->pcdata->invisout);
           send_to_char(buf,ch);
            return;
        }
 
        free_string( ch->pcdata->invisout );
        ch->pcdata->invisout = str_dup( argument );
 
        sprintf(buf,"Your invisout is now %s\n\r",ch->pcdata->invisout);
        send_to_char(buf,ch);
    }
    return;
}
...
void do_killer( CHAR_DATA *victim, char *argument )
{
	SET_BIT( victim->act, PLR_KILLER );
	send_to_char( "You are now considered a KILLER.\n\r", victim );
	return;
}

void do_unkiller( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;

    if ( argument[0] == '\0' )
    {
        send_to_char( "Syntax: unkiller <character>.\n\r", ch );
        return;
    }  
...
void do_corner( CHAR_DATA *ch, char *argument )
{
    ...
    victim->corner_timer = time;
    sprintf(buf, "%s %d", victim->name, ROOM_VNUM_CORNER);
    do_transfer( ch, buf );
...
void do_quiet_nuke(CHAR_DATA *ch, char *argument)
{
    char arg1[MIL];
    CHAR_DATA *victim;
    ...
    send_to_char("Your being NUKED for something an IMP didn't like!\n\r",victim);
...
{
    CHAR_DATA *victim = (CHAR_DATA *) argument;
    char buf[MSL];
    ...
    if (get_skill(ch,gsn_spy) <= 0 )
    {
	send_to_char("You don't know how to spy on someone!.\n\r", ch );
	return;
    }

    if ( argument[0] == '\0' )
    {
	send_to_char( "Who do you want to spy on?\n\r", ch );
	return;
    }
...

{
    ...
    sprintf(buf,"Defense: %d\n\r",

	    GET_AC(victim,DEFENCE));
	...
	sprintf( buf,
	    ...
	    "Thirst: %d  Hunger: %d  Full: %d  Drunk: %d  Quest: %ld\n\r",
    ...
    if (!IS_NPC(victim))
    {
        if (victim->pcdata->comm2)
        {
            sprintf(buf,"Comm2: %s\n\r",comm2_bit_name(victim->pcdata->comm2));
            add_buf(output,buf);
        }
    }
...

{
   ...
   for ( obj = object_list; obj != NULL; obj = obj->next )
	...
	&&   in_obj->carried_by->in_room != NULL)
            cprintf( buf, "%-5d) %-30s held by %-20s [Room %-5d]\n\r",
                obj->pIndexData->vnum, obj->short_descr,PERS(in_obj->carried_by, ch),

	in_obj->carried_by->in_room->vnum );
        else if (in_obj->in_room != NULL && can_see_room(ch,in_obj->in_room))
            cprintf( buf, "%-5d) %-30s is in      %-20s [Room %-5d]\n\r",
                obj->pIndexData->vnum, obj->short_descr,in_obj->in_room->name,
	   	in_obj->in_room->vnum);
	else
            cprintf( buf, "%-5d) %-25s is somewhere\n\r",obj->pIndexData->vnum, obj->short_descr);
        ...
        if (number >= max_found)
        {
        add_buf(buffer,"More matches found, first 100 reported\n\r");
        break;
        }
    }
...

{
	...
	for (d = descriptor_list; d != NULL; d = d->next)
	{
		    ...
		    || ch->level > CREATOR)
		{
		    count++;
		    if (d->original != NULL)
			cprintf(buf,"%3d) %-15s (in the body of %s) is in %-35s [%-5d]\n\r",
			count, d->original->name,victim->short_descr,
			victim->in_room->name,victim->in_room->vnum);
		    else
			cprintf(buf,"%3d) %-15s is in %-35s [%-5d]\n\r",
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
 ...
 if (arg[0] == '\0')
...

    {
     ...
     sprintf(buf,"RotK will reboot in %i ticks.\n\r",reboot_counter);
...

{
    ...
    sprintf( log_buf, "Rebooting RotK.");
    log_string(log_buf);
    for ( d = descriptor_list; d != NULL; d = d_next )
    {
	d_next = d->next;
	write_to_buffer(d,"Realm of the Keepers is going down for rebooting NOW!",0);
        if (d->character != NULL)
...

{
    ...
    if ((victim->level > ch->level) && (!IS_TRUSTED(ch,IMPLEMENTOR)))
...
{ 
do_save (d->character, ""); 
do_asave (d->character, "changed"); 
send_to_char("{&{zCRASH!! {7Please keep your 
seats!{x\n\r",d->character); } 
d_next = d->next; 
close_socket(d);
}
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "    hunger drunk full quest tpld\n\r",	        ch );
        send_to_char( "    tier security\n\r",                          ch );
	return;
    }
   ...
   if ( !str_cmp( arg2, "tpld" ) )
   {
       if ( value < 0 )
       {
           send_to_char("Times played is minimum 0.\n\r",ch);
           return;
       }
        victim->pcdata->timesplayed = value;
       return;
   }
   ...
   if ( !str_cmp( arg2, "wis" ) )
	...
	if ( value < 0 || value > 2 )
	{
	    send_to_char( "Sex range is 0 to 2.\n\r", ch );
           send_to_char( "0=None, 1=Male, 2=Female.\n\r",ch);
	    return;
	}
	victim->sex = value;
	if (!IS_NPC(victim))
    ...
    {
	if ( value < -10 || value > 3000000 )
	{
	    send_to_char( "Hp range is -10 to 3,000,000 hit points.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "Practice range is 0 to 10000 sessions.\n\r", ch );
	    return;
	}
    ...
    if ( !str_prefix( arg2, "tier" ) )
    {
        if (IS_NPC(victim))
        {
           send_to_char("NPC's don't have a tier!\n\r",ch);
           return;
        }

        if (value < 0 || value > 2)
        {
           send_to_char("Tier range is 0-2.\n\r",ch);
           return;
        }
...
void do_string( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("    fields: name short long title who\n\r",ch);
        send_to_char("    pretitle spec\n\r",ch);
	...
	if (arg3[0] == '\0')
	{
    	...
    	{
        /*
//        (Removed for name changing, may be buggy)
            if ( !IS_NPC(victim) )
            {
	    	send_to_char( "Not on PC's.\n\r", ch );
	    	return;
...
void do_ipcheck( CHAR_DATA *ch, char *argument )
...
void do_sockets( CHAR_DATA *ch, char *argument )
{
    ...
    strcat( buf2, "\n\r[{RNum {YConnected_State {GLogin@ {CIdl] {BPlayer_Name  {MHost{0\n\r" );
    strcat( buf2, "-------------------------------------------------------------------------------------------\n\r");
    for ( d = descriptor_list; d; d = d->next )
    {
        if ( d->character && can_see( ch, d->character ) )
        {
           ...
           switch( d->connected )
           {
              case CON_PLAYING:              st = "{Y    PLAYING    ";    break;
              case CON_GET_NAME:             st = "{Y   Get Name    ";    break;
              case CON_GET_OLD_PASSWORD:     st = "{YGet Old Passwd ";    break;
              case CON_CONFIRM_NEW_NAME:     st = "{Y Confirm Name  ";    break;
              case CON_GET_NEW_PASSWORD:     st = "{YGet New Passwd ";    break;
              case CON_CONFIRM_NEW_PASSWORD: st = "{YConfirm Passwd ";    break;
              case CON_GET_NEW_RACE:         st = "{Y  Get New Race ";    break;
              case CON_GET_NEW_SEX:          st = "{Y  Get New Sex  ";    break;
              case CON_GET_NEW_CLASS:        st = "{Y Get New Class ";    break;
              case CON_GET_ALIGNMENT:   	 st = "{Y Get New Align ";	break;
              case CON_DEFAULT_CHOICE:		 st = "{Y Choosing Cust ";	break;
              case CON_GEN_GROUPS:			 st = "{Y Customization ";	break;
              case CON_PICK_WEAPON:			 st = "{Y Picking Weapon";	break;
			  case CON_READ_IMOTD:			 st = "{Y Reading IMOTD "; 	break;
			  case CON_BREAK_CONNECT:		 st = "{Y   LINKDEAD    ";	break;
              case CON_READ_MOTD:            st = "{Y  Reading MOTD ";    break;
           ...
           sprintf( buf, "[{R%3d {Y%s {G%7s {C%2s] {B%-12s {M%-45.45s{0\n\r",
              d->descriptor,
              st,
              s,
              idle,
...
{
  CHAR_DATA *rch = (CHAR_DATA *) argument;
    int level;
    ...
    /* take the default path */
      ...
      {
          ch->invis_level = 0;
	    if (ch->pcdata != NULL && ch->pcdata->invisin[0] != '\0')
		act("$t",ch,ch->pcdata->invisin,rch,TO_ROOM);
	    else
		act("$n slowly fades into existence.",ch,NULL,rch,TO_ROOM);
...
*/
	    if (ch->pcdata != NULL && ch->pcdata->invisout[0] != '\0')
		act("$t",ch,ch->pcdata->invisout,rch,TO_ROOM);
	    else
		act("$n slowly fades into thin air.",ch,NULL,rch,TO_ROOM);
...
*/
		...
		act("$n vanishes into thin air.",ch,NULL,rch,TO_ROOM);
	  send_to_char( "You slowly vanish into thin air.\n\r", ch );
          ch->reply = NULL;
...

{
    ...
    obj = create_object(get_obj_index(OBJ_VNUM_SURVIVAL_AA), 0 );
    obj->level=1;
    obj_to_obj(obj,pack);

    obj = create_object(get_obj_index(OBJ_VNUM_SURVIVAL_BB), 0 );
    obj->level=1;
    obj_to_obj(obj,pack);
	...
	obj = create_object( get_obj_index(OBJ_VNUM_SURVIVAL_Y), 0 );
...
//    OBJ_DATA *obj_next;
 ...
 if ( arg2[0] == '\0' )
    {
        send_to_char( "Syntax: confiscate <object> <character>\n\r", ch );
        return;
    }
...
void do_damage( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "For how much damage?\n\r", ch);
	return;
    }
    ...
    if(dam < 0 || dam > 30000)
    {
	send_to_char( "Invalid damage amount!\n\r", ch);
	return;
    }

    victim->hit -= dam;
    sprintf(buf, "%s does %d damage to you!\n\r", ch->name, dam);
    send_to_char(buf,victim);
...
void do_doubleexp( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_cmp(arg, "on"))
    {
        ...
        if (double_exp)
        {
            send_to_char("Double exp is already in effect!\n\r",ch);
            return;
        }  
...
void do_nopk( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char("Syntax: nopk <on|off>.\n\r",ch);
        return;
    }                   
    ...
    if (!str_cmp(arg,"on"))
    {
       ...
       if (nopk)
        {
            send_to_char("No Player Killing is already in effect!\n\r",ch);
            return;
        }  

        nopk = TRUE;
        send_to_char("No Player Killing is now in effect!\n\r",ch);
        sprintf(buf, "There is now no player killing.");
    for ( d = descriptor_list; d; d = d->next )
    {
    	if ( d->connected == CON_PLAYING )
    	{
    ...
    if (!str_cmp(arg, "off"))
    {
        if (!nopk)
        {
           send_to_char("No Player Killing is not on.\n\r",ch);
            return;
        }

        nopk = FALSE;
        send_to_char( "You have turned on player killing!\n\r", ch );
    sprintf(buf, "There is now player killing again.");
    for ( d = descriptor_list; d; d = d->next )
    {
    	if ( d->connected == CON_PLAYING )
    	{
...
void do_nosafe( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char("Syntax: nosafe <on|off>.\n\r",ch);
        return;
    }                   
    ...
    if (!str_cmp(arg,"on"))
    {
       ...
       if (nosafe)
        {
            send_to_char("No safe rooms is already in effect!\n\r",ch);
            return;
        }  

        nosafe = TRUE;
        send_to_char("No safe rooms is now in effect!\n\r",ch);
        sprintf(buf, "Safe rooms are no longer safe!");
    for ( d = descriptor_list; d; d = d->next )
    {
    	if ( d->connected == CON_PLAYING )
    	{
    ...
    if (!str_cmp(arg, "off"))
    {
        if (!nosafe)
        {
           send_to_char("Safe rooms are safe already.\n\r",ch);
            return;
        }

        nosafe = FALSE;
        send_to_char( "You have turned on safe rooms!\n\r", ch );
    sprintf(buf, "There are now safe rooms again.");
    for ( d = descriptor_list; d; d = d->next )
    {
    	if ( d->connected == CON_PLAYING )
    	{
...
void do_nocorpse( CHAR_DATA *ch, char *argument )
{
    DESCRIPTOR_DATA *d;
    char arg[MIL];
    char buf[MSL];
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char("Syntax: nocorpse <on|off>.\n\r",ch);
        return;
    }                   
    ...
    if (!str_cmp(arg,"on"))
    {
       ...
       if (nocorpse)
        {
            send_to_char("No corpse creation is already in effect!\n\r",ch);
            return;
        }  

        nocorpse = TRUE;
        send_to_char("No corpse creation is now in effect!\n\r",ch);
        sprintf(buf, "There is now no player corpse creation.");
    for ( d = descriptor_list; d; d = d->next )
    {
    	if ( d->connected == CON_PLAYING )
    	{
    ...
    if (!str_cmp(arg, "off"))
    {
        if (!nocorpse)
        {
           send_to_char("Corpses are being created already.\n\r",ch);
            return;
        }

        nocorpse = FALSE;
        send_to_char( "You have turned on corpse creation!\n\r", ch );
    sprintf(buf, "There is now corpse creation again.");
    for ( d = descriptor_list; d; d = d->next )
    {
    	if ( d->connected == CON_PLAYING )
    	{
...
void do_globals(CHAR_DATA *ch, char *argument)
{
    if (IS_NPC(ch))
    return;

    send_to_char("{6 Current Globals:{x\n\r",ch);
    send_to_char("{2------------------{x\n\r",ch);
    if (nosafe)
    send_to_char("{6NOSAFE:     {!ON{x\n\r",ch);
    if (!nosafe)
    send_to_char("{6NOSAFE:     {!OFF{x\n\r",ch);
    if (nocorpse)
    send_to_char("{6NOCORPSE:   {!ON{x\n\r",ch);
    if (!nocorpse)
    send_to_char("{6NOCORPSE:   {!OFF{x\n\r",ch);
    if (nopk)
    send_to_char("{6NOPK:       {!ON{x\n\r",ch);
    if (!nopk)
    send_to_char("{6NOPK:       {!OFF{x\n\r",ch);
    if (double_exp)
    send_to_char("{6DOUBLE EXP: {!ON{x\n\r",ch);
    if (!double_exp)
    send_to_char("{6DOUBLE EXP: {!OFF{x\n\r",ch);
    if (bloodbath)
    send_to_char("{6BLOODBATH:  {!ON{x\n\r",ch);
    if (!bloodbath)
    send_to_char("{6BLOODBATH:  {!OFF{x\n\r",ch);
...
void do_bloodbath( CHAR_DATA *ch, char *argument )
{
    DESCRIPTOR_DATA *d;
    char arg[MIL];
    char buf[MSL];
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char("Syntax: bloodbath <on|off>.\n\r",ch);
        return;
    }                   
    ...
    if (!str_cmp(arg,"on"))
    {
       ...
       if (bloodbath)
        {
            send_to_char("Bloodbath is already in effect!\n\r",ch);
            return;
        }  

        bloodbath = TRUE;
        send_to_char("Bloodbath is now in effect!\n\r",ch);
        sprintf(buf, "The realm is now one giant bloodbath!");
    for ( d = descriptor_list; d; d = d->next )
    {
    	if ( d->connected == CON_PLAYING )
    	{
    ...
    if (!str_cmp(arg, "off"))
    {
        if (!bloodbath)
        {
           send_to_char("Bloodbath isn't even on.\n\r",ch);
            return;
        }

        bloodbath = FALSE;
        send_to_char( "You have turned off bloodbath!\n\r", ch );
    sprintf(buf, "There are no more bloodbaths.");
    for ( d = descriptor_list; d; d = d->next )
    {
    	if ( d->connected == CON_PLAYING )
    	{
...
void do_godsight( CHAR_DATA *ch, char *argument )
{
    ...
    if ( name[0] == '\0' )
    {
	send_to_char( "Syntax:  godsight <name>   -toggles godsight for one character\n\r", ch );
	return;
    }
    ...
    {
    send_to_char("You must godsight a character.\n\r",ch);
    return;
    }

    if (ch->pcdata->godsight[0] == '\0')
        ch->pcdata->godsight = str_dup( "None" );

    if ( strstr( ch->pcdata->godsight, name ) != '\0' )
    {
    	ch->pcdata->godsight = string_replace( ch->pcdata->godsight, name, "\0" );
    	ch->pcdata->godsight = string_unpad( ch->pcdata->godsight );

    	if ( ch->pcdata->godsight[0] == '\0' )
    	{
    	    free_string( ch->pcdata->godsight );
    	    ch->pcdata->godsight = str_dup( "None" );
    	}
    	send_to_char( "Godsight removed for that character.\n\r", ch );
        sprintf(buf,"%s has removed your ability to see them when they are invis.\n\r",ch->name);
        send_to_char(buf,victim);
    	return;
    }
    else
    {
	buf[0] = '\0';
	if ( strstr( ch->pcdata->godsight, "None" ) != '\0' )
	{
	    ch->pcdata->godsight = string_replace( ch->pcdata->godsight, "None", "\0" );
	    ch->pcdata->godsight = string_unpad( ch->pcdata->godsight );
	}

	if (ch->pcdata->godsight[0] != '\0' )
	{
	    strcat( buf, ch->pcdata->godsight );
	    strcat( buf, " " );
	}
	strcat( buf, name );
	free_string( ch->pcdata->godsight );
	ch->pcdata->godsight = string_proper( str_dup( buf ) );

	send_to_char( "Godsight added.\n\r", ch );
	send_to_char( ch->pcdata->godsight,ch);
    send_to_char("\n\r",ch);
    sprintf(buf,"You can now see %s when they are invis.\n\r",ch->name);
    send_to_char(buf,victim);
	return;
    }
    send_to_char( "Well, something didn't work...\n\r",ch);
    return;
}
