...
char *	const	where_name	[] =
{
    ...
    "{G<{Cworn on ear{G>{x       ",
    "{G<{Cworn on ear{G>{x       ",
    "{G<{Cworn with pride{G>{x   ",
    "{G<{Cworn with pride{G>{x   ",
    "{G<{Cworn on ankles{G>{x    ",
    "{G<{Csmoked{G>{x            "
};
...
char *	format_obj_to_char	args( ( OBJ_DATA *obj, CHAR_DATA *ch,
				    bool fShort ) );
BUFFER * show_list_to_char	args( ( OBJ_DATA *list, CHAR_DATA *ch, bool fShort, bool fShowNothing ) );
...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
    ...
    if (strlen(buf)<=0)
	strcat(buf,"This object has no description. Please inform an IMP.");
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if (!IS_NPC(victim) && !victim->desc	) strcat( buf, "{@({!Linkdead{@){x " );
	...
	if ( IS_SHIELDED(victim, SHD_GLASS)       ) buf[32] = 'G';
	if ( IS_SHIELDED(victim, SHD_HOLY)        ) buf[35] = 'O';
	if ( IS_SHIELDED(victim, SHD_WATER)       ) buf[38] = 'W';
	if ( IS_EVIL(victim)
	&& IS_AFFECTED(ch, AFF_DETECT_EVIL)     ) buf[41] = 'E';
	if ( IS_GOOD(victim)
	&&   IS_AFFECTED(ch, AFF_DETECT_GOOD)     ) buf[44] = 'G';
	if ( IS_SHIELDED(victim, SHD_SANCTUARY) || IS_SHIELDED(victim, SHD_IMPROVED_SANCTUARY)   ) buf[47] = 'S';
	if ( victim->on_quest)			    buf[50] = 'Q';
	...
	if ( IS_SHIELDED(victim, SHD_ACID)       ) strcat( buf, "({@Green Aura{x)");
	if ( IS_SHIELDED(victim, SHD_GLASS)       ) strcat( buf, "({*Shimmering Aura{x)");
	if ( IS_SHIELDED(victim, SHD_HOLY)       ) strcat( buf, "({&Glowing{x)");
	if ( IS_SHIELDED(victim, SHD_WATER)       ) strcat( buf, "({$Watery Aura{x)");
	if ( IS_EVIL(victim)
	&&   IS_AFFECTED(ch, AFF_DETECT_EVIL)     ) strcat( buf, "({RRed Aura{x)");
	if ( IS_GOOD(victim)
	&&   IS_AFFECTED(ch, AFF_DETECT_GOOD)     ) strcat( buf, "({YGolden Aura{x)");
	if ( IS_SHIELDED(victim, SHD_SANCTUARY) || IS_SHIELDED(victim, SHD_IMPROVED_SANCTUARY)   ) strcat( buf, "({WWhite Aura{x)");
	if ( victim->on_quest)			    strcat( buf, "({GQuest{x)");
    }
    if ( !IS_NPC(victim) && IS_SET(victim->act, PLR_TWIT ) )
						strcat( buf, "{x[{!S{1P{#E{3D{x]");
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    {
	sprintf( buf, "%s is surrounded by an {@acidic{x shield.\n\r", PERS(victim, ch));
	buf[0] = UPPER(buf[0]);
	add_buf(output,buf);
    }
    if ( IS_SHIELDED(victim, SHD_GLASS))
    {
	sprintf( buf, "%s is surrounded by a {*glass ball{0.\n\r", PERS(victim, ch));
	buf[0] = UPPER(buf[0]);
	add_buf(output,buf);
    }
    ...
    {
	sprintf( buf, "%s is surrounded by a {&Holy{x aura.\n\r", PERS(victim, ch));
	buf[0] = UPPER(buf[0]);
	add_buf(output,buf);
    }
    ...
    {
	sprintf( buf, "%s is surrounded by a {$water{x shield.\n\r", PERS(victim, ch));
	buf[0] = UPPER(buf[0]);
	add_buf(output,buf);
    }
...
void do_scroll(CHAR_DATA *ch, char *argument)
{
    ...
    {
	send_to_char("You must provide a reasonable number(10-100).\n\r",ch);
	return;
    }
...
void do_autotitle(CHAR_DATA *ch, char *argument)
{
    ...
    {
      send_to_char("Automatic title updating set.\n\r",ch);
...
void do_prompt(CHAR_DATA *ch, char *argument)
{
   ...
   if( !strcmp( argument, "all" ) )
      strcpy( buf, "<{!%h/%Hhp {$%mm {@%vmv {#%Xtnl {*%pp {3%gg {&%ss{0> ");
   else if( !strcmp( argument, "build" ) )
      strcpy( buf, "[%z][%R][%O][%o] (%e) ");
...
void do_spy( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->position == POS_SLEEPING )
    {
	send_to_char( "You can't spy on them, you're sleeping!\n\r", ch );
	return;
    }
    ...
    if ((IS_IMMORTAL(victim) && !IS_IMMORTAL(ch)) || (IS_IMMORTAL(victim) && IS_IMMORTAL(ch) && victim->level > ch->level))
    {
    send_to_char("They sometimes do important stuff. No looking!\n\r",ch);
    send_to_char("...Sometimes.\n\r",ch);
    return;
    }
    ...
    {
	send_to_char( "It is pitch black there... \n\r", ch );
	...
	if (victim->in_room->description != NULL && !IS_NPC(ch) && !IS_SET(ch->comm, COMM_BRIEF))
	{
	    send_to_char( "{c  ",ch);
	    send_to_char( victim->in_room->description, ch );
	    if (victim->in_room->vnum == chain)
	    {
	...
	outlist = show_list_to_char( victim->in_room->contents, ch, FALSE, FALSE );
...
if (IS_NPC(ch))
    {
	send_to_char("NPC's don't need to check their score!\n\r",ch);
        return;
    }
else
{
	...
	add_buf(output,buf);
    sprintf( buf,
	"{R ________________________________________/{x%s{x%s{x\n\r",
	ch->pcdata->pretitle,
	ch->name);
    add_buf(output,buf);
    sprintf( buf, "{R| {xYou are%s{x\n\r",
    ...
    add_buf(output,buf);
    sprintf( buf,
	"{R| {xCarrying {G%d{x/{B%d{x items at {G%d{x/{B%d{x pounds.\n\r",
    ...
    sprintf( buf, "{R ----------------------------------------------------------{x\n\r");
    add_buf(output,buf);
    sprintf( buf, "{R|    {wStr: {R%-2d{M({r%-2d{M){x         {R|      {y-{Y={WSTATS{Y={y-{x\n\r",ch->perm_stat[STAT_STR],get_curr_stat(ch,STAT_STR));
    add_buf(output,buf);
    sprintf( buf, "{R|    {wInt: {R%-2d{M({r%-2d{M){x         {R|    {cDefense:{C %5d{x\n\r",ch->perm_stat[STAT_INT],get_curr_stat(ch,STAT_INT),ch->armor[0]);
    add_buf(output,buf);
    sprintf( buf, "{R|    {wWis: {R%-2d{M({r%-2d{M){x         {R|    {cSaves:{C %7d{x{x\n\r",ch->perm_stat[STAT_WIS],get_curr_stat(ch,STAT_WIS),ch->saving_throw);
    add_buf(output,buf);
    sprintf( buf, "{R|    {wDex: {R%-2d{M({r%-2d{M){x         {R|    {cHitroll:{C %5d{x\n\r",ch->perm_stat[STAT_DEX],get_curr_stat(ch,STAT_DEX),GET_HITROLL(ch));
    add_buf(output,buf);
    sprintf( buf, "{R|    {wCon: {R%-2d{M({r%-2d{M){x         {R|    {cDamroll:{C %5d{x\n\r",ch->perm_stat[STAT_CON],get_curr_stat(ch,STAT_CON),GET_DAMROLL(ch));
    add_buf(output,buf);
    sprintf( buf, "{R ----------------------------------------------------------{x\n\r");
    add_buf(output,buf);
    ...
    {
	if (ch->qps == 1)
	    sprintf( buf, "{R| {xYou have {M%ld{x quest point.\n\r", ch->qps );
	else
	    sprintf( buf, "{R| {xYou have {M%ld{x quest points.\n\r", ch->qps );
	add_buf(output,buf);
    }
    if (ch->nextquest)
    {	if (ch->nextquest == 1)
	    sprintf(buf, "{R| {xYou can quest again in less than one minute!{x\n\r");
	else
	    sprintf(buf, "{R| {xYou can quest again in {Y%d{G minutes.{x\n\r",ch->nextquest);
    }
    else
	sprintf(buf, "{R| {xYou may quest again anytime!{x\n\r");
    add_buf(output,buf);
    ...
    {
	sprintf(buf, "{R| {0You have {W%d{0 ticks in the corner\n\r", ch->corner_timer);
	add_buf(output,buf);
    }
        sprintf( buf, "{R ----------------------------------------------------------{x\n\r");
        add_buf(output,buf);
    ...
    if (IS_SET(ch->comm,COMM_SHOW_AFFECTS))
    {
        SET_BIT(ch->act, PLR_SCORE);
        do_affects(ch,"");
        REMOVE_BIT(ch->act, PLR_SCORE);
    }
    }
}
void do_affects(CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->affected != NULL )
    {
    if (!IS_SET(ch->act,PLR_SCORE))
        {
            sprintf( buf, "{xYou are affected by the following things:{x\n\r");
            add_buf(output,buf);
	...
	for ( paf = ch->affected; paf != NULL; paf = paf->next )
	{
		...
		sprintf( buf, "{R|{x%-4s%-20s", "Aff:", skill_table[paf->type].name );

	    add_buf(output,buf);

	    if ( ch->level >= 20 )
	    {
		sprintf( buf,		         "{R|{x %s by %d ",
...
void do_whois (CHAR_DATA *ch, char *argument)
{
	...
	if (!str_prefix(arg,wch->name))
	{
		...
		case MAX_LEVEL - 0 :
		    ...
		    break;
		case MAX_LEVEL - 1 :
		    sprintf(buf2, "{3HBLD{x");
		    break;
		case MAX_LEVEL - 2 :
		    sprintf(buf2, "{&HQST{x");
		    break;
		case MAX_LEVEL - 3 :
		    sprintf(buf2, "{7ELDR{x");
		    break;
		case MAX_LEVEL - 4 :
		    sprintf(buf2, "{^CLAN{x");
		    break;
		case MAX_LEVEL - 5 :
		    sprintf(buf2, "{6BLDR{x");
		    break;
		case MAX_LEVEL - 6 :
		    sprintf(buf2, "{$DIVI{x");
		    break;
      ...
      if (wch->pcdata->tier == 2)
       {
	    ...
	    sprintf(buf, "{^-{M<{#[{2%s {&%-6s %s {*>%s{*<{#]{M>{^-{0 %s%s%s%s%s%s%s%s%s%s%s%s\n\r",
		buf2,
		...
		class,
	     ...
	     IS_SET(wch->comm, COMM_AFK) ? "[{yAFK{x] " : "",
             IS_SET(wch->act,PLR_TWIT) ? "{x[{!S{1P{#E{3D{x] " : "",
	     IS_SET(wch->act,PLR_KILLER) ? "({RKILLER{x) " : "",
             IS_SET (wch->pcdata->comm2, COMM2_RP) ? "{C({cRP{C){x " : "",
             IS_SET(wch->pcdata->comm2, COMM2_PAETS) ? "{c[{YP{yA{CE{yT{YS{c]{x " : "",
		wch->pcdata->pretitle,
...
void do_who( CHAR_DATA *ch, char *argument )
{
    ...
    output = new_buf();
    outputimm = new_buf();
    add_buf(outputimm,"{x{!+{@-{#X{@-{!+{@-{#X{@-{!+ {#Visible Immortals: {!+{@-{#X{@-{!+{@-{#X{@-{!+{0\n\r");
    add_buf(output,"{x{!+{@-{#X{@-{!+{@-{#X{@-{!+  {#Visible Mortals:  {!+{@-{#X{@-{!+{@-{#X{@-{!+{0\n\r");
    for ( d = descriptor_list; d != NULL; d = d->next )
    {
        CHAR_DATA *wch;
	char class[MAX_STRING_LENGTH];
	...
	if (!can_see(ch,wch) && (IS_SHIELDED(wch,SHD_INVISIBLE) || IS_SHIELDED(wch,SHD_CAMO)))
	    continue;
	 ...
	 */
        ...
        { 
    	    if (clan_table[wch->clan].independent)
    	    {
        		sprintf(clandat, "[{R%s{x] ", clan_table[wch->clan].short_name);
    	    }
    	    else if (is_clead(wch) && is_pkill(wch))
    	    {
        		sprintf(clandat, "[{B%s{x] ", clan_table[wch->clan].short_name);
    	    }
    	    else if (is_clead(wch) && !is_pkill(wch))
    	    {
        		sprintf(clandat, "[{M%s{x] ", clan_table[wch->clan].short_name);
    	    }
    	    else if (is_pkill(wch))
    	    {
        		sprintf(clandat, "[{b%s{x] ", clan_table[wch->clan].short_name);
    ...
    if (IS_GODSIGHT(ch,wch))
        sprintf(gsdat, "{g[{YGS{g]{x ");
    else
        sprintf(gsdat, "{x");
	...
	{
	    sprintf( buf, "{^-{M<{#[{2%s %s {*>%s{*<{#]{M>{^-{0 %s%s%s%s%s%s%s%s%s%s%s%s%s%s\n\r",
		buf2,
		...
		clandat,
        gsdat,
		((wch->ghost_level >= LEVEL_HERO)&&(ch->level >= wch->level)) ? "(G) ": "",
	...
	{
	    sprintf( buf, "{^-{M<{#[{2%s {&%-6s %s {*>%s{*<{#]{M>{^-{0 %s%s%s%s%s%s%s%s%s%s%s%s%s%s\n\r",
		buf2,
		wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name 
				    : "     ",
             	class,
	...
	if (!can_see(ch,wch) && (IS_SHIELDED(wch,SHD_INVISIBLE) || IS_SHIELDED(wch,SHD_INVISIBLE)))
	    continue;
...
void do_consider( CHAR_DATA *ch, char *argument )
{
    ...
    diff = ((victim->hit / 500) - (ch->hit / 500));
    vac = -(GET_AC(victim,DEFENCE));
    cac = -(GET_AC(ch,DEFENCE));
    diff += (vac - cac);
...
void set_pretitle( CHAR_DATA *ch, char *pretitle )
{
    ...
    if ( pretitle[0] != '.' && pretitle[0] != ',' && pretitle[0] != '!' && pretitle[0] != '?' )
    {
	buf[0] = ' ';
	strcpy( buf+1, pretitle );
    }
    else
    {
	strcpy( buf, pretitle );
...
void do_title( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf, "Title changed to %s\n\r", ch->pcdata->title );
    send_to_char(buf,ch);
}
...
/*    sprintf( buf, "Weight: %d/%d/%d (10th pounds)\n\r",
	...
	case ITEM_TOKEN:
	    sprintf( buf, "Worth{M %d{x Quest Points!\n\r", obj->value[0] );
		send_to_char( buf, ch );
		break;
...
{
send_to_char("You were unable to find anything about the object.\n\r",ch);
return;
}
}
...
/*    if (IS_SET(ch->comm,COMM_NOBEEP))
    {
	send_to_char("Your beeping ability has been disabled.\n\r",ch);
	return;
    } */
...

{
...

    {
        ...
        REMOVE_BIT(victim->comm, COMM_NOBEEP);

        send_to_char( "The gods have restored your beeping priviliges.\n\r", 

		      victim );

        send_to_char( "NOBEEP removed.\n\r", ch );

	sprintf(buf,"$N lets %s use beep again.",victim->name);
...

    {
        ...
        SET_BIT(victim->comm, COMM_NOBEEP);

        send_to_char( "The gods have revoked your beeping priviliges.\n\r", 

		       victim );

        send_to_char( "NOBEEP set.\n\r", ch );

	sprintf(buf,"$N revokes %s's beeping abilities.",victim->name);
...
{
        char arg1[MIL],group[MSL], buf[MSL];
          ...
          for (level = 1 ;level <= LEVEL_IMMORTAL;level++)
             {
                  for (sn = 0; sn < MAX_SKILL; sn++)
                  {
                  if (skill_table[sn].skill_level[cl] > 101)
                      continue;
                  if (skill_table[sn].skill_level[cl] != level)
                      continue;
                  if (skill_table[sn].spell_fun == spell_null)
                      continue;
                  sprintf(buf,"{B[{C%3d{B] {W%-18s{x",level, skill_table[sn].name);
                  send_to_char(buf,ch);
                  spCount++;
                  if (++col % 3 == 0)
                      send_to_char("\n\r",ch);
    ...
    if (((skill = skill_lookup(arg1)) != -1) && (cl = class_lookup(arg1)) == -1)
    {
        ...
        send_to_char("{CIn the group: {WUnknown{C!{x\n\r",ch);
        send_to_char("{cClass        Level Gain Wait-Time Min-Mana Rating{x\n\r",ch);
        send_to_char("{W------------ ----- ---- --------- -------- ------{x\n\r",ch);
        for (class = 0;class < MAX_CLASS;class++)
        {
        if (skill_table[skill].skill_level[class] > 101)
            continue;
        sprintf(buf,"{G%-12s {g%5d %4d %9d %8d %6d{x\n\r",
