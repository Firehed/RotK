...
break; 
case SIGSEGV: 
bug("Sig handler: Memory buffer?",0); 
do_auto_shutdown(); 
break; 
} 
} 
...
int main( int argc, char **argv )
{
    ...
    boot_db( );
    sprintf( log_buf, "RotK is ready to rock on ports %d and %d.", port, wwwport );
    log_string( log_buf );
...
#define FNDELAY O_NDELAY
     ...
     */
    dnew = new_descriptor();
    dnew->colorized     = FALSE;
    dnew->descriptor	= desc;
    dnew->connected	= CON_GET_NAME;
    dnew->ansi          = TRUE; /* color logins */
...
void read_from_buffer( DESCRIPTOR_DATA *d )
{
    ...
    if ( (k > 1 || d->incomm[0] == '!' ) && (d->connected == CON_PLAYING))
    {
...
void bust_a_prompt( CHAR_DATA *ch )
{
    ...
    bool found;
    bool round;
    const char *dir_name[] = {"{CN{0","{GE{0","{YS{0","{WW{0","{MU{0","{BD{0"};
    char *door_name[] = {"{CN{corth{0","{GE{gast{0","{YS{youth{0","{WW{west{0","{MU{mp{0","{BD{bown{0"};
    int door, outlet;
   ...
   {
	send_to_char("{G-{#<{M[{CA{cF{CK{M]{#>{G-{0 ",ch);
	return;
   }
    ...
    if (!IS_NPC(ch))
    {
    if((ch->pcdata->security >= 1) && ch->in_room != NULL )
...
void write_to_buffer( DESCRIPTOR_DATA *d, const char *txt, int length )
{
	...
	length = strlen(txt);
        ...
        /*more with color greetings*/
    if (d->connected != CON_PLAYING && d->colorized != TRUE)
    {
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
	...
	{
	    write_to_buffer( d, "{RIllegal name, try another{x.\n\r", 0 );
	...
	{
	    write_to_buffer(d,"Your site has been banned from Realm of the Keepers.\n\r",0);
	    close_socket(d);
	    return;
	}
/*
...
*/
	...
	else
	{
	    ...
	    {
		write_to_buffer( d, "Realm of the Keepers is temporarily wizlocked.\n\r", 0 );
		close_socket( d );
		return;
	    }
	}
	...
	else
	{
	    /* New player */
 	    if (newlock)
	    {
                write_to_buffer( d, "Realm of the Keepers is temporarily banning the creation of new characters.\n\r", 0 );
                close_socket( d );
                return;
            }
	    ...
	    sprintf( buf, "Did I get that right, %s?\n\r\n\r", argument );
	    write_to_buffer( d, buf, 0 );
            write_to_buffer( d, "Please keep in mind when you create a new character,\n\r",0);
            write_to_buffer( d, "{Rdo not abuse CaPiToL lEtTeRs, do not have an offensive{x\n\r",0);
            write_to_buffer( d, "{Ror racist name, and try to keep it somewhat RP{x.\n\r",0);
            write_to_buffer( d, "Confirm new character's name (Y/N)? ", 0 );
	...
	{
   	    int qpoints, bankdata, tier;
	    qpoints = ch->qps;
	    bankdata = ch->pcdata->bank;
            tier = ch->pcdata->tier;
...
/* RT code for breaking link */
	...
	for ( p = pwdnew; *p != '\0'; p++ )
	{
	    ...
	    {
		write_to_buffer( d,
		    "New password not acceptable, try again (Cannot have a tilde).\n\r{cPassword{x: ",
		    0 );
		return;
	    }
	}
...
/*            	write_to_buffer(d,race_table[race].name,0);
	...
	ch->size = pc_race_table[race].size;
        ...
        write_to_buffer( d, "\n\rWhat is your sex ({BM{x/{mF{x/{CN{x)? ", 0 );
 ...
 if (ch->pcdata->tier == 1)
  {
	    for ( iClass = MAX_CLASS/3; iClass < (MAX_CLASS/3)*2; iClass++ )
	    {
		write_to_buffer(d,"      ",0);
		write_to_buffer(d,class_table[iClass].name,0);
		write_to_buffer(d,"\n\r",1);
 ...
 if (ch->pcdata->tier == 2) {
	    for ( iClass = (MAX_CLASS/3)*2; iClass < MAX_CLASS; iClass++ )
	    {
		write_to_buffer(d,"      ",0);
		write_to_buffer(d,class_table[iClass].name,0);
		write_to_buffer(d,"\n\r",1);
	...
	if (ch->pcdata->tier == 0)
	    ...
	    if (iClass >= MAX_CLASS/3)
              {
		write_to_buffer( d,
		    "\n\rThat's not a first tier class.\n\rWhat IS your class? ", 0 );
		return;
	    }
	}
        if (ch->pcdata->tier == 1)
        {
	    if (iClass >= (MAX_CLASS/3)*2 || iClass < MAX_CLASS/3)
              {
		write_to_buffer( d,
		    "\n\rThat's not a second tier class.\n\rWhat IS your class? ", 0 );
		return;
	    }
	}
    if (ch->pcdata->tier ==2)
        {
        if (iClass < (MAX_CLASS/3)*2)
            {
            write_to_buffer ( d, "\n\rThat's not a third tier class.\n\rWhat IS your class?", 0);
            return;
            }
        }
    ch->class = iClass;
    ...
    case CON_GET_HOMETOWN:
        if (get_hometown(argument) == -1)
        {
            ...
            write_to_buffer( d, "Valid hometowns are:\n\r\n\r",0);
                sprintf(buf, "%-15s %-10s\n\r", "Town", "align");
                write_to_buffer( d, buf, 0);
	...
	write_to_buffer( d, "Which alignment ({WG{x/{CN{x/{RE{x)? ",0);
...
case CON_GET_ALIGNMENT:
	switch( argument[0])
	{
	    ...
	    default:
		write_to_buffer(d,"\n\rThat's not a valid alignment.\n\r",0);
		write_to_buffer(d,"\n\rWhich alignment ({WG{x/{CN{x/{RE{x)? ",0);
		return;
	}
	...
	write_to_buffer(d,"Customize ({YY{x/{CN{x)? ",0);
...
case CON_DEFAULT_CHOICE:
	write_to_buffer(d,"\n\r",2);
	    ...
	    list_group_costs(ch);
	    write_to_buffer(d,"You have the following skills:",0);
	    do_skills(ch,"");
	    write_to_buffer(d,"You have the following spells:",0);
	    do_spells(ch,"");
            write_to_buffer(d,"\n\r",0);
	    ...
	    write_to_buffer(d,buf,0);
            ...
            break;
        default:
            write_to_buffer( d, "Please answer ({YY{x/{CN{x)? ", 0 );
            return;
        }
	break;
	...
	ch->pcdata->learned[*weapon_table[weapon].gsn] = 40;
        write_to_buffer(d,"Do you want to use ANSI color during normal gameplay? (Y/N) ",2);
...
/*	act( "$n has entered RotK.", ch, NULL, NULL, TO_ROOM );
	do_look( ch, "auto" );*/
	...
	do_unreadnew(ch,"");
        send_to_char("\n\r",ch);
         if (IS_IMMORTAL(ch) )
          {
	wiznet("$N has entered RotK.",ch,NULL,
		WIZ_LOGINS,WIZ_SITES,get_trust(ch));
        do_look( ch, "auto" );
        }
        else
        {
//        do_announce( ch, "has entered RotK." ); taken out by Wardrof
        do_look( ch, "auto" );
        wiznet("$N has entered RotK.",ch,NULL,
                WIZ_LOGINS,WIZ_SITES,get_trust(ch));
        }
	...
	if (ch->pet != NULL)
	{
	    ...
	    act("$n has entered RotK.",ch->pet,NULL,NULL,TO_ROOM);
	}
