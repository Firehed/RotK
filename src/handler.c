...
int check_immune(CHAR_DATA *ch, int dam_type)
{
	...
	if (IS_SET(ch->imm_flags,IMM_WEAPON))
	    ...
	    def = IS_VULNERABLE;
//        else if (IS_SET(ch->abs_flags,ABS_WEAPON))
//            def = IS_ABSORBING
    }
    else /* magical attack */
    {	
	if (IS_SET(ch->imm_flags,IMM_MAGIC))
	    ...
	    def = IS_VULNERABLE;
//        else if (IS_SET(ch->abs_flags,ABS_MAGIC))
//            def = IS_ABSORBING;
    }
	...
	immune = IS_RESISTANT;
//    else if (IS_SET(ch->abs_flags,bit
...
int get_skill(CHAR_DATA *ch, int sn)
{
	...
	|| IS_SET(ch->act,ACT_RANGER) || IS_SET(ch->act,ACT_DRUID)))
	  ...
	  && IS_SET(ch->act,ACT_WARRIOR))
	...
	else if (sn == gsn_deathblow)
	    skill = 10 + 3 * (ch->level/2);

else if (sn == gsn_impale)
	    skill = 10 + 3 * (ch->level/2);

else if (sn == gsn_death_bite)
	    skill = 10 + 3 * (ch->level/2);

else if (sn == gsn_strike)
	    skill = 10 + 3 * (ch->level/2);

else if (sn == gsn_holy_fury)
	    skill = 10 + 3 * (ch->level/2);
	    ...
	    skill = 10 + 3 * (ch->level/4);

else if (sn == gsn_blind_rage)
	    skill = 10+3*(ch->level/2);
...
void reset_char(CHAR_DATA *ch)
{
    ...
    for (loc = 0; loc < MAX_WEAR; loc++)
    {
	    ...
	    ch->armor[0] += apply_ac( ch->level, obj, loc, 0 );
...
int apply_ac( int level, OBJ_DATA *obj, int iWear, int type )
{
    ...
    case WEAR_PRIDE_1:	return	   obj->value[type];  
    case WEAR_PRIDE_2:	return	   obj->value[type];  
    case WEAR_ANKLES:	return	   obj->value[type];  
    case WEAR_SMOKED:	return	   obj->value[type];
}
...
//    for (i = 0; i < 4; i++)  removed for defense code.
    	ch->armor[DEFENCE]      	+= apply_ac( ch->level, obj, iWear,0 );
    obj->wear_loc	 = iWear;
...
//    for (i = 0; i < 4; i++) removed for defense
    	ch->armor[DEFENCE]	-= apply_ac( ch->level, obj, obj->wear_loc,0 );
    obj->wear_loc	 = -1;
 ...
 */
    ...
    {
       if (ch->alignment >= 0)
           char_to_room(ch, get_room_index(hometown_table[ch->hometown].recallg));
        else
           char_to_room(ch, get_room_index(hometown_table[ch->hometown].recallb));
    }
	return;
    }
...
int get_obj_number( OBJ_DATA *obj )
{
    ...
    || obj->item_type == ITEM_JEWELRY ||  obj->item_type == ITEM_PIT)
        number = 0;
    else
        number = 1;
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    if (IS_IMMORTAL(victim) && IS_GODSIGHT(ch,victim) )
    return TRUE;
...
char *shield_bit_name( int vector )
{
    ...
    if ( vector & SHD_WATER		) strcat( buf, " water"		);
    if ( vector & SHD_ACID		) strcat( buf, " acid"		);
    if ( vector & SHD_HOLY		) strcat( buf, " holy"		);
    if ( vector & SHD_GLASS		) strcat( buf, " glass"		);
    ...
    if ( vector & SHD_CAMO              ) strcat( buf, " camouflage"    );
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
char *comm2_bit_name(int comm2_flags)
{
    static char buf[512];
    buf[0] = '\0';

    if (comm2_flags * COMM2_RP          ) strcat(buf, " rp");
...
/*    if (comm_flags & COMM_NOBEEP	) strcat(buf, " nobeep");
    if (comm_flags & COMM_NOAUCTION	) strcat(buf, " noauction"); */
...
char *wear_bit_name(int wear_flags)
{
    ...
    if (wear_flags & ITEM_WEAR_ABOUT	) strcat(buf, " aboutbody");
    ...
    if (wear_flags & ITEM_WEAR_ANKLES	) strcat(buf, " ankles");
    if (wear_flags & ITEM_WEAR_SMOKED	) strcat(buf, " smoked");
...
char *room_bit_name(int room_flags)
{
    ...
    if (room_flags & ROOM_ONEHIT    ) strcat(buf, " onehp");
    if (room_flags & ROOM_NO_QUAFF  ) strcat(buf, " no_quaff");
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
int get_hometown ( char *argument)
{
   ...
   {
        if (LOWER(argument[0]) == LOWER(hometown_table[i].name[0])
        &&  !str_prefix( argument,hometown_table[i].name))
            return i;
   }
