...
void save_char_obj( CHAR_DATA *ch )
{
	...
	fprintf(fp,"Lev %2d Trust %2d  %s%s%s\n",
	    ch->level, get_trust(ch), ch->pcdata->pretitle, ch->name, ch->pcdata->title);
	fclose( fp );
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
    ...
    if (ch->description[0] != '\0')
    	fprintf( fp, "Desc %s~\n",	ch->description	);
    if (ch->prompt != NULL || !str_cmp(ch->prompt,"<{R%h/%Hhp {B%mm {G%vmv {Y%Xtnl {8%pp {y%gg {w%ss{x>"))
        fprintf( fp, "Prom %s~\n",      ch->prompt  	);
    if (ch->pcdata->bank != 0)
	fprintf( fp, "Bank  %d~\n", ch->pcdata->bank	);
    fprintf( fp, "Corner %d\n", ch->corner_timer );
    ...
    */
	...
	if (ch->pcdata->bamfout[0] != '\0')
		fprintf( fp, "Bout %s~\n",	ch->pcdata->bamfout);
	if (ch->pcdata->invisin[0] != '\0')
	    fprintf( fp, "Invin  %s~\n",	ch->pcdata->invisin);
	if (ch->pcdata->invisout[0] != '\0')
		fprintf( fp, "Invout %s~\n",	ch->pcdata->invisout);
...
void fwrite_obj( CHAR_DATA *ch, OBJ_DATA *obj, FILE *fp, int iNest )
{
     ...
     * Castrate storage characters.
       Taken out because of random EQ.
    ...
    if (obj->pIndexData->vnum >= 3723 && obj->pIndexData->vnum <= 3750)
    {
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
    ...
    ch->comm				= COMM_COMBINE 
					| COMM_PROMPT
					| COMM_STORE;
    ch->prompt 				= str_dup("<{R%h/%Hhp {B%mm {G%vmv {Y%Xtnl {8%pp {y%gg {w%ss{x>");
    ch->pcdata->confirm_delete		= FALSE;
    ch->pcdata->pwd			= str_dup( "" );
    ch->pcdata->invisin			= str_dup( "" );
    ch->pcdata->invisout		= str_dup( "" );
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'A':
		...
		for (i = 0; i < 4; i++)
            ...
            ch->armor[DEFENCE]=ch->armor[0];
		fMatch = TRUE;
		break;
	    }

        if (!str_cmp(word,"Defense"))
	    {
//		int i;

		//for (i = 0; i < 4; i++)
		    ch->armor[0] = fread_number(fp);
            //ch->armor[DEFENCE]=(ch->armor[0]+ch->armor[1]+ch->armor[2]+ch->armor[3])/4;
		fMatch = TRUE;
		break;
	    }
	...
	case 'C':
	    ...
	    KEY( "Corner",	ch->corner_timer,	fread_number( fp ) );
	    ...
	    KEY("Comm",		ch->comm,		fread_flag( fp ) ); 
            KEY("Comm2",        ch->pcdata->comm2,      fread_flag( fp ) );
	    break;
	...
	case 'H':
        ...
        KEY( "HPC",     ch->hit,                fread_number( fp ) );
        KEY( "HPM",     ch->max_hit,            fread_number( fp ) );
        KEY( "HPP",     ch->pcdata->perm_hit,   fread_number( fp ) );
	...
	case 'I':
	    ...
	    KEY( "Invin",	ch->pcdata->invisin,	fread_string( fp ) );
	    KEY( "Invout",	ch->pcdata->invisout,	fread_string( fp ) );
	    break;
    ...
    case 'M':
        KEY( "MaC",    ch->mana,               fread_number( fp ) );
        KEY( "MaM",    ch->max_mana,           fread_number( fp ) );
        KEY( "MaP",    ch->pcdata->perm_mana,  fread_number( fp ) );
        KEY( "MvC",    ch->move,               fread_number( fp ) );
        KEY( "MvM",    ch->max_move,           fread_number( fp ) );
        KEY( "MvP",    ch->pcdata->perm_move,  fread_number( fp ) );
    break;
	...
	case 'P':
	    KEY( "Pkdt",        ch->pcdata->pkdeaths,   fread_number( fp ) );
            ...
            KEY( "Pktl",        ch->pcdata->pktotal,    fread_number( fp ) );
	...
	if ( !str_cmp( word, "Pret" ) || !str_cmp( word, "Pretitle"))
		{
		    ch->pcdata->pretitle = fread_string ( fp );
		    if (ch->pcdata->pretitle[0] != '.' && ch->pcdata->pretitle[0] != ',' && ch->pcdata->pretitle[0] != '!' && ch->pcdata->pretitle[0] != '?')
			{
	...
	case 'S':
	    ...
	    KEY( "ShD",		ch->short_descr,	fread_string( fp ) );
            ...
            KEY( "Sock",        ch->pcdata->socket,     fread_string( fp ) ); /* New finger command, should get replaced if its an actual logon */
