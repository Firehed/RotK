...
*/
    ...
    {"brother",		" {1T{7h{&e {1Bro{7th{&er{7ho{1od{x ", "{rB{wH{x",
	16200,	ROOM_VNUM_ALTAR,
	OBJ_VNUM_PIT,
	FALSE,	TRUE,	"Chaotic Good"},

    {"beholders",		" {YB{ye{rh{Yo{yl{rd{Ye{yr{rs {gof the {DShadows{x ", "{YB{DS{x",
        16221,        ROOM_VNUM_ALTAR,
        16233,
        FALSE,   TRUE,   "Chaotic Neutral" },
    ...
    {"guards",		" {yT{Ya{yl{Ya{yg{Yr{ya{Yt{yh{Yi{ya{Yn {rGu{Ra{rrd{x ",  "{YT{rG{x",
        17400,        ROOM_VNUM_ALTAR,
        OBJ_VNUM_PIT,
        FALSE,   TRUE,   "Guards of Talagrath - Peace" },
    ...
    {"covenant",		" {1Cove{8nant {!o{*f {1Pa{8in{x ",          "{1CP{x",
        22610,        ROOM_VNUM_ALTAR,
        OBJ_VNUM_PIT,
        FALSE,   TRUE,   "Chaotic Evil" },
    ...
    {"order",		" {rT{wh{re {wO{rr{wd{re{wr ",   "{rO{wR",
        22630,        ROOM_VNUM_ALTAR,
        22630,
        FALSE,   TRUE,   "Neutral Evil" },
...
void do_clead( CHAR_DATA *ch, char *argument )
{
   ...
   if (victim->class <= MAX_CLASS/3)
        {
        send_to_char("They must be at least second tier to be clan leader.\n\r",ch);
        return;
        }

   if (victim->level < 50)
        {
        send_to_char("They need to be level 50 or above!\n\r",ch);
        return;
        }
...
void do_member( CHAR_DATA *ch, char *argument )
{
	...
	if (is_clead(victim)) {
        ...
        send_to_char("They are now removed from your clan and lonered.\n\r",ch);
        send_to_char("Your clan leader has kicked you out!\n\r",victim);
        send_to_char("You are now a loner.  If you want out of being lonered,\n\ryou must reroll.\n\r",victim);
	update_clanlist(victim, victim->clan, FALSE, FALSE);
        victim->clan = 1;
        return;
    }
    ...
    if ( (victim->pcdata->tier == 0 ) && ( victim->level < 25 || victim->level > 101 ) )
    {
	send_to_char( "They must be between levels 25 -> 101.\n\r",ch);
	return;
    }
    if ( (victim->pcdata->tier == 1) && ( victim->level < 15 || victim->level > 101 ) )
    {
	send_to_char( "They must be between levels 15 -> 101.\n\r",ch);
	return;
    }
    if ( (victim->pcdata->tier == 2) && ( victim->level > 101 ) )
    {
        send_to_char( "They cannot be immortal!!\n\r",ch);
        return;
    }
...
void do_cgossip( CHAR_DATA *ch, char *argument )
{
    ...
    else  /* cgossip message sent, turn cgossip on if it isn't already */
    {
      ...
      sprintf( buf, "{2You {#C{3gossip {^'{M%s{^'{0\n\r", argument );
      send_to_char( buf, ch );
...
void do_cleadgossip( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    DESCRIPTOR_DATA *d;
    int wtime;
    ...
    if (!is_clead(ch))
    {
        send_to_char("You're not a clan leader!\n\r",ch);
        return;
    }
      ...
      sprintf( buf, "{2You {#Clead{3gossip {^'{M%s{^'{0\n\r", argument );
      send_to_char( buf, ch );
      for ( d = descriptor_list; d != NULL; d = d->next )
      {
        CHAR_DATA *victim;
	...
	bool found = FALSE;
	    ...
	    if (!found)
	    {
		act_new( "{2$n {#Clead{3gossips {^'{M$t{^'{0",
		    ch,argument, d->character, TO_VICT,POS_SLEEPING );
	    }
        }
      }
...
void do_clanlist( CHAR_DATA *ch, char *argument )
{
    ...
    output = new_buf();
//    sprintf(buf,"  '*' = PKill Clan  ' ' = Non-PKill Clan.\n\r");
//   add_buf(output,buf);
	...
	if (str_cmp(clan_table[e].exname, "Unused"))
	{
     ...
     if (IS_IMMORTAL(ch))
		...
		add_buf(output,buf);
		cprintf( buf, "{xVnum: {Y%-5d{x\n\r",
		    clan_table[e].hall );
		add_buf(output,buf);
	    }
	    if (clan_table[e].pkill)
		cprintf( buf, "{B%-20s{xMembers: {G%-3d{x\n\rDesc: {c%-30s{x\n\r",
		    clan_table[e].who_name,
		    members,
		    clan_table[e].exname );
	    else
		cprintf( buf, "{M%-20s{xMembers: {G%-3d{x\n\rDesc: {c%-30s{x\n\r",
		    clan_table[e].who_name,
		    members,
		    clan_table[e].exname );
	    add_buf(output,buf);
	    cprintf(buf,"Leaders:{R");
	    add_buf(output,buf);
	    for (pcln = cln_list; pcln != NULL; pcln = pcln->next)
	    {
		if (pcln->clan == e)
