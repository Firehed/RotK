...
void move_char( CHAR_DATA *ch, int door, bool follow, bool quiet )
{
    ...
    if (IS_SET(ch->in_room->room_flags, ROOM_ONEHIT))
        ch->hit = 1;
...
void do_visible( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "You can now be seen.\n\r", ch );
    return;
}
void do_badrecall( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    ROOM_INDEX_DATA *pRoomIndex;
    int loss, exploss, ldloss, damamount;

        /* Taken from WizSlap */
        pRoomIndex = get_random_room(ch);
        /* No Talagrath BR's */
        if (((pRoomIndex->vnum >= 17000) && (pRoomIndex->vnum <= 17599))
        ...
        || (IS_SET(!IS_IMMORTAL(ch) && pRoomIndex->room_flags, ROOM_GODS_ONLY))
        || (IS_SET(pRoomIndex->room_flags, ROOM_NOQUEST))
        || (IS_SET(pRoomIndex->room_flags, ROOM_IMP_ONLY && ch->level < MAX_LEVEL)))
        {
        do_badrecall(ch, "");
        return;
        }
        ...
        act( "$n scrapes $S knees from landing on the ground in an unexpected place!", ch, NULL, NULL, TO_ROOM );

        /* Larger EXP Loss! */
        if (ch->level <= MAX_LEVEL-9)
            {
                exploss = 2 * ch->level;
                ldloss = ch->level;
                loss = (ch->desc != NULL) ? exploss : ldloss;
        	gain_exp( ch, 0 - loss );
                if (ch->desc != NULL)
                {
                    sprintf(buf, "You have lost %d exp points!\n\r", exploss);
                    send_to_char(buf,ch);
                }
             }
        /* Hurt 'em */
        damamount = ch->hit * .1;
        ch->hit -= damamount;
        sprintf(buf, "Your bad recall does %d damage to you!!\n\r\n\r", damamount );
        send_to_char(buf,ch);
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->alignment < 0 )
    {
	if ( ( location = get_room_index( hometown_table[ch->hometown].recallb ) ) == NULL )
	{
    ...
    {
	if ( ( location = get_room_index( hometown_table[ch->hometown].recallg ) ) == NULL )
	{
	    send_to_char( "You are completely lost.\n\r", ch );
	    return;
	}
    ...
    if ( (number_range(1,100) > get_skill(ch,gsn_recall))
        && (ch->level > 25))
    {
        do_badrecall(ch, "" );
        return;
	...
	if ( number_percent() < 80 * skill / 100 )
	{
        ...
        if ( number_range(1,100) > ((get_skill(ch,gsn_recall)*2)/3)
        /* Newbie-Proof it */
        && (ch->level > 25))
        {
        do_badrecall(ch, "");
        ...
        exploss=ch->level*10;
        ldloss=ch->level*5;
	lose = (ch->desc != NULL) ? exploss : ldloss;
	gain_exp( ch, 0 - lose );
...
void do_htrecall( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    CHAR_DATA *victim;
    ROOM_INDEX_DATA *location;
    ...
    if (!is_clan(ch))
    {
        send_to_char("Just recall normally!\n\r",ch);
        return;
    }
