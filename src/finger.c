...
void do_finger( CHAR_DATA *ch, char *argument )
{
    ...
    /* Acutal time of logoff */
    sprintf(buf,"%s last logged off on %s",
	victim->name, (char *) ctime(&victim->llogoff));
    send_to_char(buf,ch);
    /* Some data */
   sprintf(buf, "Level: {c%d{x  Class: {g%s{x  Race: {w%s{x  Sex: %s{x\n\r", victim->level, class_table[victim->class].name, race_table[victim->race].name, victim->sex == 0 ? "{cSexless" : victim->sex == 1 ? "{BMale" : "{mFemale");
   send_to_char(buf,ch);

    /* Clan Stuff */
    if (is_clan(victim))
    {
    sprintf(buf,"Member of clan [%s{x]", clan_table[victim->clan].who_name);
    send_to_char(buf,ch);
        if(is_clead(victim))
        {
        send_to_char(" and is clan leader",ch);
        }
    send_to_char(".\n\r",ch);
    }

    /* IMMs can see last socket (For bans) */
    if (IS_IMMORTAL(ch))
    {
    sprintf(buf, "Last played from %s\n\r", victim->pcdata->socket);
    send_to_char(buf,ch);
    }
    free_pcdata(victim->pcdata);
    free_char(victim);
