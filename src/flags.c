...
void do_flag(CHAR_DATA *ch, char *argument)
{
	...
	send_to_char("  char flags: plr,comm,comm2,aff,shd,imm,res,vuln\n\r",ch);
	...
	if (!str_prefix(arg1,"mob") || !str_prefix(arg1,"char"))
	{
		...
		flag_table = part_flags;
            ...
            else if (!str_prefix(arg3,"comm2"))
            {
                if (IS_NPC(victim))
                {
                   send_to_char("Comm2 flags aren't for NPCs.\n\r",ch);
                   return;
                }
                flag_table = comm2_flags;
            }
...
/* Mobiles and Characters */
	...
	else if (!str_prefix(arg3,"comm2"))
	{
	    if (IS_NPC(victim))
	    {
		send_to_char("Comm2 can't be set on NPCs.\n\r",ch);
		return;
	    }

	    flag = &victim->pcdata->comm2;
	    flag_table = comm2_flags;
	}
