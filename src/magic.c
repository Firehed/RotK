...
* Spell functions.
*/
void spell_choke( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;

    if ( IS_AFFECTED(victim, AFF_CHOKE))
    {
    ...
    af.modifier  = -4;
    af.duration  = 3;
    af.bitvector = AFF_CHOKE;
    affect_to_char( victim, &af );
    send_to_char( "You are choked!\n\r", victim );
    act("$n appears to be choked.\n\r",victim,NULL,NULL,TO_ROOM);
    ch->hit -= 500;
    return;
}


void spell_summon_ant( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
    ...
    dam = dice( ch->level, (ch->level*.25) );
    damage_old( ch, victim, dam, sn,DAM_FIRE,TRUE);
    return;
}

void spell_summon_rabbit( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
    ...
    dam = dice( ch->level, (ch->level*.5) );
    damage_old( ch, victim, dam, sn,DAM_COLD,TRUE);
   return;
}

void spell_summon_wolf( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
    ...
    dam = dice( ch->level, ch->level*.75 );
    damage_old( ch, victim, dam, sn,DAM_ACID,TRUE);
    return;
}

void spell_summon_tiger( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
    ...
    dam = dice( ch->level, ch->level );
    damage_old( ch, victim, dam, sn,DAM_COLD,TRUE);
    return;
}

void spell_summon_dragon( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
   ...
   if ( ( ch->fighting == NULL )
    ...
    dam = dice( ch->level, (ch->level*1.75) );

    damage_old( ch, victim, dam, sn,DAM_FIRE,TRUE);
    return;
}

void spell_summon_golem( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
...
void spell_soul_rift( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
...
void spell_final_blessing( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    {
	if (victim == ch)
	  send_to_char("You are already blessed by gods.\n\r",ch);
	else
	  act("$N is already blessed by gods.",ch,NULL,victim,TO_CHAR);
	return;
    }
        /* AC */
    af.where	 = TO_AFFECTS;
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
    send_to_char( "You feel blessed by gods.\n\r", victim );
    if ( ch != victim )
	act("$N is blessed by your magic.",ch,NULL,victim,TO_CHAR);
    return;
}
...
void spell_calm( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if (IS_IMMORTAL(ch)) /* always works for imms*/
      mlevel = 0;
...
void spell_chill_touch( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    dam		= number_range( ch->level, ch->level*2 )+30;
...
#define OBJ_VNUM_ENERGY_BED 35
/*
 * for the below spell
 */

void spell_energy_bed(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    OBJ_DATA *bed;

    bed = create_object( get_obj_index(OBJ_VNUM_ENERGY_BED), 1);
    obj_to_room( bed, ch->in_room );
    act( "$n utters a spell and $p appears.", ch, bed, NULL, TO_ROOM );
    act( "You utter a spell and $p appears.", ch, bed, NULL, TO_CHAR );
    bed->timer = 5;
    return;
}
...
void spell_control_weather(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    send_to_char( "Weather controlled.\n\r", ch );
    return;
}
...

{
	...
	send_to_char( "Critical wounds have been healed.\n\r", ch );
...

{
	...
	send_to_char( "Light wounds have been healed.\n\r", ch );
...

{
	...
	send_to_char( "Serious wounds have been healed.\n\r", ch );
...
void spell_detect_evil( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if ( ch != victim )
	send_to_char( "They can detect evil now.\n\r", ch );
    return;
}
...
void spell_detect_good( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    send_to_char( "Your eyes tingle.\n\r", victim );
    if ( ch != victim )
        send_to_char( "They can now detect good.\n\r", ch );
    return;
}
...
void spell_detect_hidden(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    if ( ch != victim )
	send_to_char( "They can now see the hidden.\n\r", ch );
    return;
}
...
void spell_detect_invis( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    send_to_char( "Your eyes tingle.\n\r", victim );
    if ( ch != victim )
	send_to_char( "They can now see the invisible.\n\r", ch );
    return;
}
...
void spell_detect_magic( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    send_to_char( "Your eyes tingle.\n\r", victim );
    if ( ch != victim )
	send_to_char( "They can now sense magic.\n\r", ch );
    return;
}
...
void spell_dispel_evil( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if ( IS_GOOD(victim) )
    {
	act( "Wardrof protects $N.", ch, NULL, victim, TO_ROOM );
	return;
    }
    ...
    if (victim->hit > (ch->level * 4))
      ...
      dam = UMAX(victim->hit, dice(level,10));
    if ( saves_spell( level, victim,DAM_HOLY) )
	dam /= 2;
    if ( ( ch->fighting == NULL )
    && ( !IS_NPC( ch ) )
...

{
...

    {
        ...
        act( "Firehed protects $N.", ch, NULL, victim, TO_ROOM );
...
void spell_dispel_magic( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (found)
        send_to_char("Dispel spell worked!\n\r",ch);
    else
        send_to_char("Spell failed.\n\r",ch);
	return;
}
...
void spell_acidstorm( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    send_to_char( "The sky falls with acid rain!\n\r", ch );
    act( "$n makes the sky shoot acid.", ch, NULL, NULL, TO_ROOM );
   ...
   if (is_same_group(vch,ch) )
		    ...
		    damage_old( ch,vch,(level + dice(level, level*3))*get_curr_stat(ch,STAT_WIS)/20, sn, DAM_ACID,TRUE);
	    }
	    continue;
	}

	if ( vch->in_room->area == ch->in_room->area )
	    send_to_char( "In the distace you can see the sky shoot forth acid.\n\r", vch );
    }
...
void spell_holyfire( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *vch;
    CHAR_DATA *vch_next;

    send_to_char( "The room is filled with holy fire!\n\r", ch );
    act( "$n makes the room fill with holy fire.", ch, NULL, NULL, TO_ROOM );
   ...
   if (is_same_group(vch,ch) )
		    ...
		    damage_old( ch,vch,(level + dice(level, level*2.5))*get_curr_stat(ch,STAT_WIS)/20, sn, DAM_FIRE,TRUE);
	    }
	    continue;
	}

	if ( vch->in_room->area == ch->in_room->area )
	    send_to_char( "In the distace you can see the sky shoot forth holy fire.\n\r", vch );
    }
...
void spell_shadowfire( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *vch;
    CHAR_DATA *vch_next;

    send_to_char( "The room is filled with dark fire!\n\r", ch );
    act( "$n makes the room fill with dark fire.", ch, NULL, NULL, TO_ROOM );
   ...
   if (is_same_group(vch,ch) )
	...
	if ( vch->in_room->area == ch->in_room->area )
	    send_to_char( "In the distace you can see the sky shoot forth dark fire.\n\r", vch );
    }
...
void spell_random_ring(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    OBJ_DATA *ring;
    AFFECT_DATA *pAf;
    int hitapp,damapp,hpapp,manaapp,atlevel,wearloc,ac,dama,damb,damdesc,magicclass;
    int added,saveapp,lighttime,con,dex,intell,str,wis,weight,newmagic;
    int arrowa, arrowb, arrowswitch;
    char actxt[MSL];
    char mc[MSL];
    char mc2[MSL];
    char mc3[MSL];
    ...
    wearloc=number_range(1,30);
    atlevel=number_range(1,101);


    added=number_range(1,100);
    if (added > 25)
    hitapp=number_range(1,atlevel*.75);//75 //worth 30
    else
    hitapp=0;

    added=number_range(1,100);
    if (added > 25)
    damapp=number_range(1,atlevel*.75);//75 //worth 10
    else
    damapp=0;

    added=number_range(1,100);
    if (added > 25)
    hpapp=number_range(1,atlevel*6.5);//656 //worth 50
    else
    hpapp=0;

    added=number_range(1,100);
    if (added > 25)
    manaapp=number_range(1,atlevel*6.5);//656 //worth 100
    else
    manaapp=0;

    added=number_range(1,100);
    if (added > 25)
    saveapp=number_range(1,atlevel*.60);//60  //worth 10
    else
    saveapp=0;
...
con=0;
    ...
    added=number_range(1,4);
    if (added == 1)
    {
        added=number_range(1,10);
        if (added==1)
        {
        con=1;
        sprintf(suffix," of vitality");
        }

        if (added==2)
        {
        con=2;
        sprintf(suffix," of constitution");
        }

        if (added==3)
        {
        str=1;
        sprintf(suffix," of vigor");
        }

        if (added==4)
        {
        str=2;
        sprintf(suffix," of strength");
        }

        if (added==5)
        {
        wis=1;
        sprintf(suffix," of knowledge");
        }

        if (added==6)
        {
        wis=2;
        sprintf(suffix," of wisdom");
        }

        if (added==7)
        {
        intell=1;
        sprintf(suffix," of intellect");
        }

        if (added==8)
        {
        intell=2;
        sprintf(suffix," of intelligence");
        }

        if (added==9)
        {
        dex=1;
        sprintf(suffix," of quickness");
        }

        if (added==10)
        {
        dex=2;
        sprintf(suffix," of dexterity");
        }
    }
    else
    sprintf(suffix,"");

    ac=number_range(1,atlevel);
    dama=number_range(1,atlevel*.25);
    damb=number_range(1,atlevel*.30);
    damdesc=dama+damb;
    magicclass=hitapp+damapp+manaapp+hpapp+saveapp;

    newmagic=hitapp/3+damapp+manaapp/10+hpapp/5+saveapp;
    
    lighttime = number_range(atlevel*50, atlevel*100);//10.1K
    if (lighttime > 4000)
    {
    lighttime = -1;
    }
    ...
    {
    sprintf(mc,"weak");
    sprintf(mc2,"{y orange");
    sprintf(mc3,"might");
    arrowa = 1; arrowb = 0;
    }
    else if (newmagic < 188)
    {
    sprintf(mc,"magical");
    sprintf(mc2,"{Y yellow");
    sprintf(mc3,"salvation");
    arrowa = 0; arrowb = 1;
    }
    else if (newmagic < 281)
    {
    sprintf(mc,"strong");
    sprintf(mc2,"{G green");
    sprintf(mc3,"concentration");
    arrowa = 1; arrowb = 1;
    }
    else if (newmagic <328)
    {
    sprintf(mc,"powerful");
    sprintf(mc2,"{B blue");
    sprintf(mc3,"fantacism");
    arrowa = 2; arrowb = 1;
    }
    else
    {
    sprintf(mc,"superb");
    sprintf(mc2,"{m violet");
    sprintf(mc3,"vengence");
    arrowa = 2; arrowb = 2;
    }
    ...
    {
    sprintf(actxt,"cloth");
    sprintf(floattxt,"hazy");
    weight=1;
    metal = FALSE;
    }
    else if (ac <=34)
    {
    sprintf(actxt,"leather");
    sprintf(floattxt,"dim");
    metal=FALSE;
    weight=3;
    }
    else if (ac <=51)
    {
    sprintf(actxt,"studded leather");
    sprintf(floattxt,"shimmering");
    metal=FALSE;
    weight=5;
    }
    else if (ac <=68)
    {
    sprintf(actxt,"iron");
    sprintf(floattxt,"bright");
    metal=TRUE;
    weight=20;
    }
    else if (ac <= 85)
    {
    sprintf(actxt,"titanium");
    sprintf(floattxt,"brilliant");
    metal=TRUE;
    weight=15;
    }
    else
    {
    sprintf(actxt,"mithril");
    sprintf(floattxt,"dazzling");
    metal=FALSE;
    weight=5;
    }
    ring = NULL;
    ...
    {
	case 1:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_RING ), 0 );
    weight *=.5;
	ring->value[0] = ac/2;
	ring->value[1] = ac/2;
	ring->value[2] = ac/2;
	ring->value[3] = ac/2;
        sprintf(name, "random %s ring%s",mc,suffix);
        sprintf(shdesc, "A %s ring%s",mc,suffix);
        sprintf(lodesc, "A %s ring%s is lying on the ground here.",mc,suffix);
    metal=FALSE;
	break;

	case 2:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_AMULET ), 0 );
	ring->value[0] = ac/2;
	ring->value[1] = ac/2;
	ring->value[2] = ac/2;
	ring->value[3] = ac/2;
    weight *=.5;
        sprintf(name, "random %s amulet%s",mc,suffix);
        sprintf(shdesc, "A %s amulet%s",mc, suffix);
        sprintf(lodesc, "A %s amulet%s is lying on the ground here.",mc,suffix);
    metal=FALSE;
	break;

	case 3:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_SLEEVES ), 0 );    
	ring->value[0] = ac;
	ring->value[1] = ac;
	ring->value[2] = ac;
	ring->value[3] = ac;
        sprintf(name, "random %s %s sleeves%s",mc,actxt,suffix);
        sprintf(shdesc, "A pair of %s %s sleeves%s",mc,actxt,suffix);
        sprintf(lodesc, "A pair of %s %s sleeves%s are lying on the ground here.",mc,actxt,suffix);
        break;

	case 4:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_ARMOR ), 0 );
	ring->value[0] = ac*4;
	ring->value[1] = ac*4;
	ring->value[2] = ac*4;
	ring->value[3] = ac*4;
    weight *=2;
        sprintf(name, "random %s %s armor%s",mc,actxt,suffix);
        sprintf(shdesc, "Some %s %s armor%s",mc,actxt,suffix);
        sprintf(lodesc, "A set of %s %s armor%s is lying on the ground here.",mc,actxt,suffix);
	break;

	case 5:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_BOOTS ), 0 );
	ring->value[0] = ac;
	ring->value[1] = ac;
	ring->value[2] = ac;
	ring->value[3] = ac;
        sprintf(name, "random %s %s boots%s",mc,actxt,suffix);
        sprintf(shdesc, "A pair of %s %s boots%s",mc,actxt,suffix);
        sprintf(lodesc, "A pair of %s %s boots%s are lying on the ground here.",mc,actxt,suffix);
	break;

	case 6:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_GLOVES ), 0 );
	ring->value[0] = ac;
	ring->value[1] = ac;
	ring->value[2] = ac;
	ring->value[3] = ac;
        sprintf(name, "random %s %s gloves%s",mc,actxt,suffix);
        sprintf(shdesc, "A pair of %s %s gloves%s",mc,actxt,suffix);
        sprintf(lodesc, "A pair of %s %s gloves%s are lying on the ground here.",mc,actxt,suffix);
	break;

        case 7:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_SWORD ), 0 );
	ring->value[1] = dama;
	ring->value[2] = damb;
    weight *=2;
        if (damdesc==2)
        ...
        sprintf(name, "random %s %s sword%s",mc,dam,suffix);
        sprintf(shdesc, "A %s %s sword%s",mc,dam,suffix);
        sprintf(lodesc, "A %s %s sword%s is lying on the ground here.",mc,dam,suffix);
    metal=FALSE;
	break;

	case 8:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_HELMET ), 0 );
	ring->value[0] = ac*2;
	ring->value[1] = ac*2;
	ring->value[2] = ac*2;
	ring->value[3] = ac*2;
        sprintf(name, "random %s %s helmet%s",mc,actxt,suffix);
        sprintf(shdesc, "A %s %s helmet%s",mc,actxt,suffix);
        sprintf(lodesc, "A %s %s helmet%s is lying on the ground here.",mc,actxt,suffix);
	break;

	case 9:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_MASK ), 0 );
	ring->value[0] = ac;
	ring->value[1] = ac;
	ring->value[2] = ac;
	ring->value[3] = ac;
        sprintf(name, "random %s %s mask%s",mc,actxt,suffix);
        sprintf(shdesc, "A %s %s mask%s",mc,actxt,suffix);
        sprintf(lodesc, "A %s %s mask%s is lying on the ground here.",mc,actxt,suffix);
	break;

	case 10:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_PANTS ), 0 );
	ring->value[0] = ac*2;
	ring->value[1] = ac*2;
	ring->value[2] = ac*2;
	ring->value[3] = ac*2;
        sprintf(name, "random %s %s pants%s",mc,actxt,suffix);
        sprintf(shdesc, "A pair of %s %s pants%s",mc,actxt,suffix);
        sprintf(lodesc, "A pair of %s %s pants%s are lying on the ground here.",mc,actxt,suffix);
	break;

	case 11:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_SHIELD ), 0 );
	ring->value[0] = ac*3;
	ring->value[1] = ac*3;
	ring->value[2] = ac*3;
	ring->value[3] = ac*3;
    weight *=2;
        sprintf(name, "random %s %s shield%s",mc,actxt,suffix);
        sprintf(shdesc, "A %s %s-enforced shield%s",mc,actxt,suffix);
        sprintf(lodesc, "A %s %s-enforced shield%s lying on the ground here.",mc,actxt,suffix);
	break;

	case 12:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_BRACER ), 0 );
	ring->value[0] = ac/2;
	ring->value[1] = ac/2;
	ring->value[2] = ac/2;
	ring->value[3] = ac/2;
    weight *=.5;
        sprintf(name, "random %s %s bracer%s",mc,actxt,suffix);
        sprintf(shdesc, "A %s %s bracer%s",mc,actxt,suffix);
        sprintf(lodesc, "A %s %s bracer%s lying on the ground here.",mc,actxt,suffix);
	break;

	case 13:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_ANKLET ), 0 );
	ring->value[0] = ac/2;
	ring->value[1] = ac/2;
	ring->value[2] = ac/2;
	ring->value[3] = ac/2;
    weight *=.5;
        sprintf(name, "random %s %s anklets%s",mc,actxt,suffix);
        sprintf(shdesc, "Some %s %s anklets%s",mc,actxt,suffix);
        sprintf(lodesc, "A pair of %s %s anklets%s lying on the ground here.",mc,actxt,suffix);
    metal=FALSE;
	break;

	case 14:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_EARRING ), 0 );
	ring->value[0] = ac/2;
	ring->value[1] = ac/2;
	ring->value[2] = ac/2;
	ring->value[3] = ac/2;
    weight *=.5;
        sprintf(name, "random %s earring%s",mc,suffix);
        sprintf(shdesc, "A %s earring%s",mc,suffix);
        sprintf(lodesc, "A %s earring%s is lying on the ground here.",mc,suffix);
    metal=FALSE;
	break;

	case 15:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_ORB ), 0 );
	ring->value[0] = ac/2;
	ring->value[1] = ac/2;
	ring->value[2] = ac/2;
	ring->value[3] = ac/2;
    weight *=.5;
        sprintf(name, "random %s{x orb%s",mc2,suffix);
        sprintf(shdesc, "A%s{x orb%s",mc2,suffix);
        sprintf(lodesc, "A%s{x orb%s is lying on the ground here.",mc2,suffix);
    metal=FALSE;
	break;

    case 16:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_DAGGER ), 0 );
	ring->value[1] = dama;
	ring->value[2] = damb;
    weight *=2;
        if (damdesc==2)
        sprintf(dam,"tiny dagger");
        else if (damdesc<16)
        sprintf(dam,"small dagger");
        else if (damdesc <31)
        sprintf(dam,"medium dagger");
        else if (damdesc<41)
        sprintf(dam,"large dagger");
        else if (damdesc<51)
        sprintf(dam,"kris");
        else
        sprintf(dam,"stiletto");

        sprintf(name, "random %s %s%s",mc,dam,suffix);
        sprintf(shdesc, "A %s %s%s",mc,dam,suffix);
        sprintf(lodesc, "A %s %s%s is lying on the ground here.",mc,dam,suffix);
    metal=FALSE;
	break;

    case 17:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_BODY ), 0 );
	ring->value[0] = ac;
	ring->value[1] = ac;
	ring->value[2] = ac;
	ring->value[3] = ac;
        sprintf(name, "random %s %s cloak%s",mc,actxt,suffix);
        sprintf(shdesc, "A %s %s cloak%s",mc,actxt,suffix);
        sprintf(lodesc, "A %s %s cloak%s is lying on the ground here.",mc,actxt,suffix);
	break;

    case 18:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_WAIST ), 0 );
	ring->value[0] = ac/3;
	ring->value[1] = ac/3;
	ring->value[2] = ac/3;
	ring->value[3] = ac/3;
        sprintf(name, "random %s %s sash%s",mc,actxt,suffix);
        sprintf(shdesc, "A %s %s sash%s",mc,actxt,suffix);
        sprintf(lodesc, "A %s %s sash%s is lying on the ground here.",mc,actxt,suffix);
	break;

    case 19:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_FLOAT ), 0 );
	ring->value[0] = ac*1.5;
	ring->value[1] = ac*1.5;
	ring->value[2] = ac*1.5;
	ring->value[3] = ac*1.5;
        sprintf(name, "random %s %s aura spreader%s",floattxt,mc3,suffix);
        sprintf(shdesc, "A %s %s aura spreader%s",floattxt,mc3,suffix);
        sprintf(lodesc, "A %s %s aura spreader%s is floating here.",floattxt,mc3,suffix);
    metal=FALSE;
	break;

    case 20:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_LIGHT ), 0 );
        ring->value[2] = lighttime;
        sprintf(name, "random %s lantern%s",mc,suffix);
        sprintf(shdesc, "A %s lantern%s",mc,suffix);
        sprintf(lodesc, "A %s lantern%s is lying on the ground here.",mc,suffix);
    metal=FALSE;
	break;

    case 21:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_STAFF ), 0 );
	ring->value[1] = dama + 4;
	ring->value[2] = damb + 4;
    weight *=2;
        if (damdesc==2)
        sprintf(dam,"broken staff");
        else if (damdesc<16)
        sprintf(dam,"wooden staff");
        else if (damdesc <31)
        sprintf(dam,"hardwood staff");
        else if (damdesc<41)
        sprintf(dam,"hollow steel staff");
        else if (damdesc<51)
        sprintf(dam,"forged iron staff");
        else
        sprintf(dam,"mithril staff");
        if (hitapp)
        hitapp *= 1.1;
        if (manaapp)
        manaapp *= 1.1;
    ...
    case 22:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_MACE ), 0 );
	ring->value[1] = dama;
	ring->value[2] = damb;
    weight *=2;
        if (damdesc==2)
        sprintf(dam,"nearly broken maul");
        else if (damdesc<16)
        sprintf(dam,"small maul");
        else if (damdesc <31)
        sprintf(dam,"medium maul");
        else if (damdesc<41)
        sprintf(dam,"large maul");
        else if (damdesc<51)
        sprintf(dam,"great maul");
        else
        sprintf(dam,"thunder maul");
    ...
    case 23:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_AXE ), 0 );
	ring->value[1] = dama;
	ring->value[2] = damb;
    weight *=2;
        if (damdesc==2)
        sprintf(dam,"dented axe");
        else if (damdesc<16)
        sprintf(dam,"dull axe");
        else if (damdesc <31)
        sprintf(dam,"sharp axe");
        else if (damdesc<41)
        sprintf(dam,"razor-sharp axe");
        else if (damdesc<51)
        sprintf(dam,"jagged axe");
        else
        sprintf(dam,"serrated axe");
    ...
    case 24:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_FLAIL ), 0 );
	ring->value[1] = dama;
	ring->value[2] = damb;
    weight *=2;
        if (damdesc==2)
        sprintf(dam,"tiny flail");
        else if (damdesc<16)
        sprintf(dam,"small flail");
        else if (damdesc <31)
        sprintf(dam,"large flail");
        else if (damdesc<41)
        sprintf(dam,"iron flail");
        else if (damdesc<51)
        sprintf(dam,"steel flail");
        else
        sprintf(dam,"mithril flail");
    ...
    case 25:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_WHIP ), 0 );
	ring->value[1] = dama;
	ring->value[2] = damb;
    weight *=2;
        if (damdesc==2)
        sprintf(dam,"tiny whip");
        else if (damdesc<16)
        sprintf(dam,"short whip");
        else if (damdesc <31)
        sprintf(dam,"long whip");
        else if (damdesc<41)
        sprintf(dam,"bullwhip");
        else if (damdesc<51)
        sprintf(dam,"chain whip");
        else
        sprintf(dam,"cat-of-nine-tails");
    ...
    case 26:
    weight *=2;
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_POLEARM ), 0 );
	ring->value[1] = dama + 5;
	ring->value[2] = damb + 5;
        if (damdesc==2)
        sprintf(dam,"dented polearm");
        else if (damdesc<16)
        sprintf(dam,"dull polearm");
        else if (damdesc <31)
        sprintf(dam,"sharp polearm");
        else if (damdesc<41)
        sprintf(dam,"razor-sharp polearm");
        else if (damdesc<51)
        sprintf(dam,"jagged polearm");
        else
        sprintf(dam,"serrated polearm");
    ...
    case 27:
	ring = create_object( get_obj_index( OBJ_VNUM_RANDOM_BOW ), 0 );
	ring->value[1] = dama + 5;
	ring->value[2] = damb + 5;
    weight *=2;
        if (damdesc==2)
        sprintf(dam,"short bow");
        else if (damdesc<16)
        sprintf(dam,"long bow");
        else if (damdesc <31)
        sprintf(dam,"war bow");
        else if (damdesc<41)
        sprintf(dam,"ballista");
        else if (damdesc<51)
        sprintf(dam,"hydra bow");
        else
        sprintf(dam,"grand matron bow");
    ...
    ring = create_object(get_obj_index(OBJ_VNUM_RANDOM_QUIVER),0);
    ring->value[0] = ac*10; /*Current Arrows*/
    ring->value[1] = arrowa;/*Dice 1 Bonus*/
    ring->value[2] = arrowb;/*Dice 2 Bonus*/
    ring->value[3] = ac*10; /*Max Arrows*/

    arrowswitch = arrowa + arrowb;
    if (arrowswitch == 0)
    sprintf(dam, "pinewood arrows");
    else if (arrowswitch == 1)
    sprintf(dam, "maplewood arrows");
    else if (arrowswitch == 2)
    sprintf(dam, "ironwood arrows");
    else if (arrowswitch == 3)
    sprintf(dam, "broadhead arrows");
    else
    sprintf(dam, "silver arrows");
    ...
    {
    sprintf(quiver, "small");
    weight=0;
    }
    else if (ac < 81)
    {
    sprintf(quiver, "medium");
    weight=0;
    }
    else if (ac < 91)
    {
    sprintf(quiver, "large");
    weight=0;
    }
    else
    {
    ...
    sprintf(name,  "random quiver %s %s",quiver,dam);
    sprintf(shdesc,"%s quiver of %s",quiver,dam);
    sprintf(lodesc,"A %s quiver of %s is on the ground here.",quiver,dam);
    break;
    default: return;


     }
    if (ring->item_type != ITEM_QUIVER)
    {
    ...
    if (metal)
    {
    if (!IS_SET(ring->extra_flags, ITEM_METAL))
        SET_BIT(ring->extra_flags, ITEM_METAL);
    }

    else
    {
    if (IS_SET(ring->extra_flags, ITEM_METAL))
        REMOVE_BIT(ring->extra_flags, ITEM_METAL);
    }

    free_string(ring->name);
    free_string(ring->short_descr);
    free_string(ring->description);

    ring->name = str_dup( name );
    ring->short_descr = str_dup( shdesc );
    ring->description = str_dup( lodesc );
    ring->level = atlevel;
    ring->cost = magicclass*ac*10;
    ring->weight =weight;

    obj_to_char(ring, ch);

    send_to_char("You made a randomized object!\n\r",ch);
    return;
}
...
/*    if ( ch != victim )
        send_to_char( "They are moving a lot faster.\n\r", ch ); */
    return;
}
...

{
	...
	if (obj->clan)
	{
	    sprintf( buf, "Quiver stats are variable.\n\r");
	} else
	{
	    sprintf( buf,
...
void spell_camo( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
...
//    victim = (CHAR_DATA *) vo;
    ...
    if ( IS_SHIELDED(victim, SHD_CAMO) )
	return;

    act( "$n {Wfa{wde{Ds in{cto the {yba{cck{ggr{cou{ynd.{x", victim, NULL, NULL, TO_ROOM );
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = SHD_CAMO;
    affect_to_char( victim, &af );
    send_to_char( "You {wfa{Dde {cinto t{yhe b{cack{ggro{cun{yd.{x\n\r", victim );
    return;
}
...
void spell_locate_object( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
	...
	||   ch->level < obj->level || IS_SET(obj->extra_flags,ITEM_NOLOCATE) )
	    continue;
...
void spell_magic_missile( int sn, int level, CHAR_DATA *ch,void *vo,int target)
{
    ...
    dam		= number_range( ch->level, ch->level*2 )+20;
    if ( ( ch->fighting == NULL )
    && ( !IS_NPC( ch ) )
    && ( !IS_NPC( victim ) ) )
    {
...
void spell_improved_sanctuary( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;

    if ( IS_SHIELDED(victim, SHD_SANCTUARY) || IS_SHIELDED(victim, SHD_IMPROVED_SANCTUARY) )
    {
	if (victim == ch)
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = SHD_IMPROVED_SANCTUARY;
    affect_to_char( victim, &af );
...
void spell_summon( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    ||   (IS_NPC(victim) && saves_spell( level, victim,DAM_OTHER))
    || (!IS_NPC(victim) && !is_same_clan(ch,victim) && victim->in_room->area != ch->in_room->area) )
...
void spell_acid_breath( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    dam = dice(ch->level*.8,ch->level*.8)*2;
...
void spell_frost_breath( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    dam = dice(ch->level*.8,ch->level*.9)*2;
...
void spell_gas_breath( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    dam = dice(ch->level,ch->level*.9)*2;
...
void spell_lightning_breath(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    dam = dice(ch->level*.9,ch->level*.9)*2;
...
void spell_make_bag( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    ...
    obj->pIndexData = get_obj_index( OBJ_VNUM_CORPSEBAG );    /* So it's not a corpse */
