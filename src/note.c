...
void do_unread(CHAR_DATA *ch)
{
    ...
    {
	found = TRUE;
	sprintf(buf,"There %s %d new {Bwedding{x announcement%s waiting.\n\r",
    ...
    {
	found = TRUE;
	sprintf(buf,"There %s %d new {BRolePlay{x note%s waiting.\n\r",
	    count > 1 ? "are" : "is",count, count > 1 ? "s" : "");
	send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"There %s %d new {Rnews{x article%s waiting.\n\r",
	    count > 1 ? "are" : "is",count, count > 1 ? "s" : "");
	send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"There %s %d {Ychange%s{x waiting to be read.\n\r",
	    count > 1 ? "are" : "is", count, count > 1 ? "s" : "");
        send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"You have %d new {Wnote%s{x waiting.\n\r",
	    count, count > 1 ? "s" : "");
	send_to_char(buf,ch);
    }
    ...
    {
	found = TRUE;
	sprintf(buf,"You have %d unread {widea%s{x to peruse.\n\r",
	    count, count > 1 ? "s" : "");
	send_to_char(buf,ch);
    }
	...
	sprintf(buf,"%d %s been added.\n\r",
	    count, count > 1 ? "{Mpenalties{x have" : "{Mpenalty{x has");
	send_to_char(buf,ch);
    }
...
void do_unreadnew(CHAR_DATA *ch)
{
    char buf[MAX_STRING_LENGTH];
    int count;
	...
	sprintf(buf, "{g| {RNEWS{g         | {D%-3d{g |{x\n\r",count);
	send_to_char(buf,ch);

	count=count_spool(ch,note_list);
	sprintf(buf, "{g| {WNOTES{g        | {D%-3d{g |{x\n\r",count);
	send_to_char(buf,ch);

	count=count_spool(ch,idea_list);
	sprintf(buf, "{g| {wIDEAS{g        | {D%-3d{g |{x\n\r",count);
	send_to_char(buf,ch);

	count=count_spool(ch,changes_list);
	sprintf(buf, "{g| {YCHANGES{g      | {D%-3d{g |{x\n\r",count);
	send_to_char(buf,ch);

	count=count_spool(ch,rpnote_list);
	sprintf(buf, "{g| {mRP NOTES{g     | {D%-3d{g |{x\n\r",count);
	send_to_char(buf,ch);

	count=count_spool(ch,weddings_list);
	sprintf(buf, "{g| {BWEDDINGS{g     | {D%-3d{g |{x\n\r",count);
	send_to_char(buf,ch);

	if (IS_IMMORTAL(ch))
	{
	count=count_spool(ch,penalty_list);
	sprintf(buf, "{g| {MPENALTIES{g    | {D%-3d{g |{x\n\r",count);
	send_to_char(buf,ch);
	}
	send_to_char("{g======================{x\n\r",ch);
    return;
...
void parse_note( CHAR_DATA *ch, char *argument, int type )
{
	...
	default:
        ...
        case NOTE_RPNOTE:
            list = &rpnote_list;
	    list_name = "role play notes";
            list_name2 = "{mrole play note{x";
            break;
        case NOTE_IDEA:
            list = &idea_list;
	    list_name = "ideas";
        ...
        case NOTE_CHANGES:
            list = &changes_list;
	    list_name = "changes";
            list_name2 = "{Ychange{x";
            break;
	case NOTE_WEDDINGS:
	    list = &weddings_list;
	    list_name = "weddings";
            list_name2 = "{Bwedding{x";
	    break;
    }
	...
	if (!vnum)
	{
	    ...
	    {
		case NOTE_NOTE:	
		    sprintf( buf, "There are no {Wnotes{x for you.\n\r");
		    break;
		case NOTE_RPNOTE:
		    sprintf( buf, "There are no {mrole play notes{x for you.\n\r");
		    break;
		case NOTE_IDEA:
		    sprintf( buf, "There are no {wideas{x for you.\n\r");
		    break;
		case NOTE_PENALTY:
		    sprintf( buf, "There are no {Mpenalties{x for you.\n\r");
		    break;
		case NOTE_NEWS:
		    sprintf( buf, "There are no {Rnews{x for you.\n\r");
		    break;
		case NOTE_CHANGES:
		    sprintf( buf, "There are no {Ychanges{x for you.\n\r");
		    break;
		case NOTE_WEDDINGS:
		    sprintf( buf, "There are no {Bwedding{x announcements for you.\n\r");
		    break;
	    }
	    add_buf(output,buf);
	}
	...
	switch(type)
	{
		...
		ch->pcdata->last_note = current_time;
                send_to_char( "You are caught up on your notes.\n\r",ch);
		break;
	    case NOTE_RPNOTE:
		ch->pcdata->last_rpnote = current_time;
                send_to_char( "You are caught up on your RP notes.\n\r",ch);
		break;
	    case NOTE_IDEA:
		ch->pcdata->last_idea = current_time;
                send_to_char( "You are caught up on your ideas.\n\r",ch);
		break;
	    case NOTE_PENALTY:
		ch->pcdata->last_penalty = current_time;
                send_to_char( "You are caught up on your penalties.\n\r",ch);
		break;
	    case NOTE_NEWS:
		ch->pcdata->last_news = current_time;
                send_to_char( "You are caught up on your news.\n\r",ch);
		break;
	    case NOTE_CHANGES:
		ch->pcdata->last_changes = current_time;
                send_to_char( "You are caught up on your changes.\n\r",ch);
		break;
	    case NOTE_WEDDINGS:
		ch->pcdata->last_weddings = current_time;
                send_to_char( "You are caught up on your wedding announcements.\n\r",ch);
		break;
	}
	return;
    }
...
/*   below this point only certain people can edit notes */
    if ((type == NOTE_NEWS && !IS_TRUSTED(ch,SQUIRE))
    ||  (type == NOTE_CHANGES && !IS_TRUSTED(ch,SQUIRE))
	...
	{
	    send_to_char(
		"You already have a different type of note in progress.\n\r",ch);
	    return;
	}
	...
	{
	    if((d->connected == CON_PLAYING) && (d->character->name != ch->name))
	    {
                if(is_name(d->character->name, ch->pnote->to_list))
	        {
				...
				{
				case NOTE_NOTE:
	            	send_to_char("You have a private {Wnote{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_RPNOTE:
	            	send_to_char("You have a private {mrole play note{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_IDEA:
	            	send_to_char("You have a private {widea{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_PENALTY:
	            	send_to_char("You have a private {Mpenalty{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_NEWS:
	            	send_to_char("You have a private {Rnews{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_CHANGES:
	            	send_to_char("You have some private {Ychanges{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_WEDDINGS:
	            	send_to_char("You have a private new {Bwedding{x announcement to read.\n\r{x", d->character);
	            	break;
				default:
	            	send_to_char("You have a private new message.\n\r{x", d->character);
	            	break;
				}
		}
        	if((d->character->name != ch->name) && (is_name("all", ch->pnote->to_list)
        	|| (((d->character->level >= LEVEL_IMMORTAL) && (is_name("immortal", ch->pnote->to_list))))))
                        {
				switch(ch->pnote->type)
				{
				case NOTE_NOTE:
	            	send_to_char("There is a new {Wnote{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_RPNOTE:
	            	send_to_char("There is a new {mrole play note{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_IDEA:
	            	send_to_char("There is a new {widea{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_PENALTY:
	            	send_to_char("There is a new {Mpenalty{x to read.\n\r{x", d->character);
	            	break;
				case NOTE_NEWS:
	            	send_to_char("There is a new {Rnews{x for you to read.\n\r{x", d->character);
	            	break;
				case NOTE_CHANGES:
	            	send_to_char("There is a new {Ychange{x for you to read!\n\r{x", d->character);
	            	break;
                        	case NOTE_WEDDINGS:
	            	send_to_char("There is a new {Bwedding{x announcement for you to read!\n\r{x", d->character);
	            	break;
			
				default:
	            	send_to_char("You have a new message.\n\r{x", d->character);
	            	break;
				}
                        }
	    }
        ...
        sprintf(buf, "Your %s has been posted.\n\r",list_name2);
        send_to_char(buf,ch);
	ch->pnote->next			= NULL;
