...
void advance_level( CHAR_DATA *ch )
{
    ...
    sprintf( buf,
	"Your gain is: {!%d{1({!%d{1)HP{0, {$%d{4({$%d{4)Mana{0, {@%d{2({@%d{2)MV{0 {^%d{6({^%d{6)Pracs{0.\n\r",
	add_hp,		ch->max_hit,
...
//	send_to_char( "You raise a level!!  ",ch);
	ch->level += 1;
	sprintf(buf,"{WYou obtain level {#%d{0!!  ",ch->level);
	send_to_char(buf,ch);
...
if (ch->level >= 50)
    ...
    ch->move = ch->max_move;
	update_pos(ch);
	send_to_char("You have been given the power to {Rc{Yo{Gn{Bt{Ci{mn{Mu{We{D!{0\n\r",ch);
	save_char_obj(ch);
}    
else
{
...
void weather_update( void )
{
	...
	strcat( buf, "{cThe day has begun.{x\n\r" );
	break;

    case  6:
	weather_info.sunlight = SUN_RISE;
	strcat( buf, "{cThe {Ysun{c rises in the east.{x\n\r" );
	break;

    case 12:
        strcat( buf, "{cEveryone pauses for a second as {yhigh noon{c comes around.{x\n\r");
        break;

    case 19:
	weather_info.sunlight = SUN_SET;
	strcat( buf, "{cThe {Ysun{c slowly disappears in the west{x.\n\r" );
	break;

    case 20:
	weather_info.sunlight = SUN_DARK;
	strcat( buf, "{cThe {Dnight{c has begun.{x\n\r" );
	break;

    case 24:
        strcat( buf, "{cYou hear a howl in the distance as {Dm{wi{Dd{wn{Di{wg{Dh{wt{c ticks by.{x\n\r");
	time_info.hour = 0;
	time_info.day++;
	break;
    }
	...
	{
	    strcat( buf, "The sky is getting {wcloudy{x.\n\r" );
	...
	{
	    strcat( buf, "It starts to {Brain{x.\n\r" );
	    weather_info.sky = SKY_RAINING;
	}

	if ( weather_info.mmhg > 1030 && number_bits( 2 ) == 0 )
	{
	    strcat( buf, "The {wclouds{x disappear.\n\r" );
	    weather_info.sky = SKY_CLOUDLESS;
	}
	break;
	...
	|| ( weather_info.mmhg > 1010 && number_bits( 2 ) == 0 ) )
	{
	    strcat( buf, "The {Brain{x stopped.\n\r" );
	    weather_info.sky = SKY_CLOUDY;
	}
	break;
...
{   
	...
	if ( ch->corner_timer == 1 )
	{
	 char_from_room(ch);
	 char_to_room(ch, get_room_index(ROOM_VNUM_QUEST));
	 do_look(ch, "auto");
	}
...
global_exp--;
    if (global_exp == 0)
    {
        ...
        {
    	if ( d->connected == CON_PLAYING )
    	    send_to_char("Double exp as expired!\n\r", d->character );
        }
     double_exp = FALSE;
    }
...
void update_handler( void )
{
    ...
    static  int     pulse_auto_save;
    static  int     pulse_per_sec;

    if ( --pulse_auto_save <= 0)
    {
    pulse_auto_save= PULSE_AUTO_SAVE;
//    auto_save ();
    }
    ...
    if ( --pulse_per_sec <= 0)
    {
        pulse_per_sec = PULSE_PER_SECOND;
        regen_update();
    }
